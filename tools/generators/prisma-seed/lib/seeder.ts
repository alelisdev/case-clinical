...
export class Seeder {
  ...
  async importFromCSV(model: string, fileInputName: string, include?){
    
    let collection = csvToJson.fieldDelimiter('\t').formatValueByType().getJsonFromCsv(fileInputName);
    
    console.debug(`collection ${collection.toString()}`)
    // collection.array.forEach(row => {
