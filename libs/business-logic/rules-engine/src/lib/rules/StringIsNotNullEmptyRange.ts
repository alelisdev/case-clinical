...
export class StringIsNotNullEmptyRange extends CompositeRule {
  ...
  /**
   * Use to indicate the minimum length of the target value.
   */
  minLength: number;
   ...
   */
  constructor(name: string, message: string, target: string, minLength: number, maxLength: number, isDisplayable = false) {
  ...
  configureRules() {
    ...
    if (this.target != null) {
      this.rules.push(
        new Range('TargetLengthIsWithinRange', 'The string value is not within the specified range.', this.target.toString().length, this.minLength, this.maxLength)
      );
    }
  }
}
