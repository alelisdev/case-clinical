...
export class WebFormsUiFeeScheduleComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebFeeScheduleFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'feeSchedule_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.feeSchedule = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'feeSchedule_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadFeeScheduleEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.feeSchedule = vm.item})
      }
    }
