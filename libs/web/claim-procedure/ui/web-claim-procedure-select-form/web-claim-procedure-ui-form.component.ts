...
export class WebFormsUiClaimProcedureComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebClaimProcedureFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'claimProcedure_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.claimProcedure = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'claimProcedure_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadClaimProcedureEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.claimProcedure = vm.item})
      }
    }
