...
import { WebCoreDataAccessService, FirmStatus, CorePaging, UserUpdateFirmStatusInput  } from '@case-clinical/web/core/data-access'
...
export interface FirmStatusListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebFirmStatusListStore extends ComponentStore<FirmStatusListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addFirmStatuses = this.updater((state, firmStatuses: any[]) => ({...state, data: state.data.concat(firmStatuses) }))
    updateFirmStatuses = this.updater((state, firmStatuses: any[]) => {
        return {
            ...state,
            data: state.data.map((firmStatus) => {
            const updated = firmStatuses.find((el) => el.id === firmStatus.id);
