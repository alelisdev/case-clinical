...
@Component({
  selector: 'ui-request-additional-visit-form',
  template: `
...
export class WebFormsUiRequestAdditionalVisitComponent implements OnInit, OnDestroy {
  @Input() requestAdditionalVisit: RequestAdditionalVisit
  ...
  constructor(
    public readonly store: WebRequestAdditionalVisitFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'requestAdditionalVisit_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.requestAdditionalVisit = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'requestAdditionalVisit_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadRequestAdditionalVisitEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.requestAdditionalVisit = vm.item})
      }
    }
