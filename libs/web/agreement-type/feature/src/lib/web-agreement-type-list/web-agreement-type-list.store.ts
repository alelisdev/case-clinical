...
import { WebCoreDataAccessService, AgreementType, CorePaging, UserUpdateAgreementTypeInput  } from '@case-clinical/web/core/data-access'
...
export interface AgreementTypeListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebAgreementTypeListStore extends ComponentStore<AgreementTypeListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addAgreementTypes = this.updater((state, agreementTypes: any[]) => ({...state, data: state.data.concat(agreementTypes) }))
    updateAgreementTypes = this.updater((state, agreementTypes: any[]) => {
        return {
            ...state,
            data: state.data.map((agreementType) => {
            const updated = agreementTypes.find((el) => el.id === agreementType.id);
