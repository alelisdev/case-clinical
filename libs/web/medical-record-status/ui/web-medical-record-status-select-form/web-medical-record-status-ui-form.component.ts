...
@Component({
  selector: 'ui-medical-record-status-form',
  template: `
...
export class WebFormsUiMedicalRecordStatusComponent implements OnInit, OnDestroy {
  @Input() medicalRecordStatus: MedicalRecordStatus
  ...
  constructor(
    public readonly store: WebMedicalRecordStatusFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'medicalRecordStatus_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({name: ''})
      this.store.vm$.subscribe((vm) => {
      this.medicalRecordStatus = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'medicalRecordStatus_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadMedicalRecordStatusEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.medicalRecordStatus = vm.item})
      }
    }
