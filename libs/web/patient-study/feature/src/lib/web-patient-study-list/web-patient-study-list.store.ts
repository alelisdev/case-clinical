...
import { WebCoreDataAccessService, PatientStudy, CorePaging, UserUpdatePatientStudyInput ,Patient } from '@case-clinical/web/core/data-access'
...
export interface PatientStudyListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebPatientStudyListStore extends ComponentStore<PatientStudyListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addPatientStudies = this.updater((state, patientStudies: any[]) => ({...state, data: state.data.concat(patientStudies) }))
    updatePatientStudies = this.updater((state, patientStudies: any[]) => {
        return {
            ...state,
            data: state.data.map((patientStudy) => {
            const updated = patientStudies.find((el) => el.id === patientStudy.id);
  ...
  validateImportData(excelData: any[]) {
    return this.vm$.pipe(
      switchMap((vm) => {
        ...
        return this.patientStudyService.validatePatientStudyExcelData(excelData,patients);
      })
    )
  }
