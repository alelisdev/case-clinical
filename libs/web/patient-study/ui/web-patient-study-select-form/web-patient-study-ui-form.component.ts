...
export class WebFormsUiPatientStudyComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebPatientStudyFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'patientStudy_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.patientStudy = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'patientStudy_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPatientStudyEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.patientStudy = vm.item})
      }
    }
