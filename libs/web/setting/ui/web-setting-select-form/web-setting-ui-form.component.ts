...
export class WebFormsUiSettingComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebSettingFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'setting_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.setting = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'setting_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadSettingEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.setting = vm.item})
      }
    }
