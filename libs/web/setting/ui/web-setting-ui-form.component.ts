...
import { WebSettingFormStore } from './web-setting-form.store'
...
WebUiFormField.input('dateFormat', { label: 'Date Format' }, {className: 'sm:w-full md:w-1/2 lg:w-1/4  px-1'}),
WebUiFormField.input('timeFormat', { label: 'Time Format' }, {className: 'sm:w-full md:w-1/2 lg:w-1/4  px-1'}),
WebUiFormField.input('startWeekOn', { label: 'Start Week On' }, {className: 'sm:w-full md:w-1/2 lg:w-1/4  px-1'})				])

]

  constructor(
    private readonly store: WebSettingFormStore,
    private readonly route: ActivatedRoute,
    private readonly ref: ChangeDetectorRef
  ) {}


  async submit({ name,dateFormat,timeFormat,startWeekOn,userId }) {
    
    if(this.parentUserId != ''){
      userId = this.parentUserId
    }

    await this.store.createSettingEffect({ name,dateFormat,timeFormat,startWeekOn,userId })
