...
import { WebSettingDetailStore } from './web-setting-detail.store'
import {
...
@Component({
  ...
  providers: [WebSettingDetailStore],
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class WebSettingDetailComponent implements AfterViewInit {
  @ViewChild('matDrawer', { static: true }) matDrawer: MatDrawer
  drawerMode: 'side' | 'over'
  drawerOpened: boolean
  menuData: FuseNavigationItem[] = []
  private _unsubscribeAll: Subject<any> = new Subject<any>()
  private _item: Setting
  readonly vm$ = this.store.vm$

  constructor(
    private readonly store: WebSettingDetailStore,
