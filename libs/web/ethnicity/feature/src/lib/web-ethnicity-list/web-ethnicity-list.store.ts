...
import { WebCoreDataAccessService, Ethnicity, CorePaging, UserUpdateEthnicityInput  } from '@case-clinical/web/core/data-access'
...
export interface EthnicityListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebEthnicityListStore extends ComponentStore<EthnicityListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addEthnicities = this.updater((state, ethnicities: any[]) => ({...state, data: state.data.concat(ethnicities) }))
    updateEthnicities = this.updater((state, ethnicities: any[]) => {
        return {
            ...state,
            data: state.data.map((ethnicity) => {
            const updated = ethnicities.find((el) => el.id === ethnicity.id);
