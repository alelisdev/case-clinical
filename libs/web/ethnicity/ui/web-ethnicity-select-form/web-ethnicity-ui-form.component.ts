...
export class WebFormsUiEthnicityComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebEthnicityFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'ethnicity_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.ethnicity = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'ethnicity_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadEthnicityEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.ethnicity = vm.item})
      }
    }
