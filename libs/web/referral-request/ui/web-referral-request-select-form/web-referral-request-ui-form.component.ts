...
@Component({
  selector: 'ui-referral-request-form',
  template: `
...
export class WebFormsUiReferralRequestComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebReferralRequestFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'referralRequest_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.referralRequest = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'referralRequest_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadReferralRequestEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.referralRequest = vm.item})
      }
    }
