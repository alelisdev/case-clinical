...
export class WebFormsUiPchProviderComponent implements OnInit, OnDestroy {
  @Input() pchProvider: PchProvider
  ...
  constructor(
    public readonly store: WebPchProviderFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'pchProvider_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.pchProvider = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'pchProvider_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPchProviderEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.pchProvider = vm.item})
      }
    }
