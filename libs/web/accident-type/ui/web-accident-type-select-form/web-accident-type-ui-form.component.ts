...
@Component({
  selector: 'ui-accident-type-form',
  template: `
...
export class WebFormsUiAccidentTypeComponent implements OnInit, OnDestroy {
  @Input() accidentType: AccidentType
  ...
  constructor(
    public readonly store: WebAccidentTypeFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'accidentType_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({name: ''})
      this.store.vm$.subscribe((vm) => {
      this.accidentType = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'accidentType_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadAccidentTypeEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.accidentType = vm.item})
      }
    }
