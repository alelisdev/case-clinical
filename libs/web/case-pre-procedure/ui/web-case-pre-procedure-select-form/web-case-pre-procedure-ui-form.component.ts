...
export class WebFormsUiCasePreProcedureComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebCasePreProcedureFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'casePreProcedure_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.casePreProcedure = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'casePreProcedure_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadCasePreProcedureEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.casePreProcedure = vm.item})
      }
    }
