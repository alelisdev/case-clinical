...
import { WebCoreDataAccessService, CasePreProcedure, UserCreateCasePreProcedureInput, LegalCase } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebCasePreProcedureFormStore extends ComponentStore<CasePreProcedureFormState> {
...
{debounce: true})
  ...
  readonly createCasePreProcedureEffect = this.effect<UserCreateCasePreProcedureInput>((input$) =>
    input$.pipe(
      ...
      switchMap((input) =>
        this.data.userCreateCasePreProcedure({ input }).pipe(
          tapResponse(
            (res) => {
