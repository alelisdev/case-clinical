...
import { WebCoreDataAccessService, Tag, CorePaging } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebTagListStore extends ComponentStore<TagListState> {
  ...
  private mapDataToItems(data: Tag[]) {
    return (data || []).map(
      ({ name, id, createdAt }) =>
        ({
          title: name,
