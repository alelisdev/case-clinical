...
import { WebCoreDataAccessService, Navigation, CorePaging } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebNavigationListStore extends ComponentStore<NavigationListState> {
  ...
  private mapDataToItems(data: Navigation[]) {
    return (data || []).map(
      ({ name, id, createdAt }) =>
        ({
          title: name,
