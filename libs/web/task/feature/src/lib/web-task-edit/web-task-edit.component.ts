...
import { UserUpdateTaskInput, Role ,Appointment } from '@case-clinical/web/core/data-access'
import { WebUiFormField } from '@case-clinical/web/ui/form'
import { WebTaskEditStore } from './web-task-edit.store'
import { ActivatedRoute, Router } from '@angular/router'
import { FormlyFieldConfig } from '@ngx-formly/core'
import { map } from 'rxjs/operators'

@Component({
  templateUrl: './web-task-edit.component.html',
  providers: [WebTaskEditStore],
})
export class WebTaskEditComponent {
...
WebUiFormField.input('entity', { label: 'Entity' }, {className: 'w-full  px-1'}),
  ...
  constructor(
    private readonly store: WebTaskEditStore,
    private route: ActivatedRoute,
    private router: Router,
  ) {}

  updateTask(input: UserUpdateTaskInput) {
     const { name,title,dueDate,assignedDate,completedOn,completed,completionNotes,assignedToId,appointmentId,taskCategoryId,taskSubCategoryId,taskStatus,taskPriorityName,assignedUser,subject,summary,dueBy,scheduledFor,dateClosed,closedBy,isImportant,temp,createdBy,dateCreated,removed,taskCompletedDate,migSource,entity } = input
     this.store.updateTaskEffect({ name,title,dueDate,assignedDate,completedOn,completed,completionNotes,assignedToId,appointmentId,taskCategoryId,taskSubCategoryId,taskStatus,taskPriorityName,assignedUser,subject,summary,dueBy,scheduledFor,dateClosed,closedBy,isImportant,temp,createdBy,dateCreated,removed,taskCompletedDate,migSource,entity })
  }
