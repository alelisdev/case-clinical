...
@Component({
  selector: 'ui-favorite-provider-form',
  template: `
...
export class WebFormsUiFavoriteProviderComponent implements OnInit, OnDestroy {
  @Input() favoriteProvider: FavoriteProvider
  ...
  constructor(
    public readonly store: WebFavoriteProviderFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'favoriteProvider_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.favoriteProvider = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'favoriteProvider_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadFavoriteProviderEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.favoriteProvider = vm.item})
      }
    }
