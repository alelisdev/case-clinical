...
export class WebFormsUiPriorAuthDmeComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebPriorAuthDmeFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'priorAuthDme_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.priorAuthDme = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'priorAuthDme_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPriorAuthDmeEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.priorAuthDme = vm.item})
      }
    }
