...
import { WebCoreDataAccessService, VendorType, CorePaging, UserUpdateVendorTypeInput  } from '@case-clinical/web/core/data-access'
...
export interface VendorTypeListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebVendorTypeListStore extends ComponentStore<VendorTypeListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addVendorTypes = this.updater((state, vendorTypes: any[]) => ({...state, data: state.data.concat(vendorTypes) }))
    updateVendorTypes = this.updater((state, vendorTypes: any[]) => {
        return {
            ...state,
            data: state.data.map((vendorType) => {
            const updated = vendorTypes.find((el) => el.id === vendorType.id);
