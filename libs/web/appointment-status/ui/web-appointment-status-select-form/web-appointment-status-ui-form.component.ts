...
@Component({
  selector: 'ui-appointment-status-form',
  template: `
...
export class WebFormsUiAppointmentStatusComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebAppointmentStatusFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'appointmentStatus_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.appointmentStatus = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'appointmentStatus_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadAppointmentStatusEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.appointmentStatus = vm.item})
      }
    }
