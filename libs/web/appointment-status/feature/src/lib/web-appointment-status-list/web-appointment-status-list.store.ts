...
import { WebCoreDataAccessService, AppointmentStatus, CorePaging, UserUpdateAppointmentStatusInput  } from '@case-clinical/web/core/data-access'
...
export interface AppointmentStatusListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebAppointmentStatusListStore extends ComponentStore<AppointmentStatusListState> {
  constructor(
    ...
    super({
      headerTitle: 'AppointmentStatuses',
      searchFocused: false,
      searchQuery: '',
...
readonly vm$ = this.select(
    ...
    addAppointmentStatuses = this.updater((state, appointmentStatuses: any[]) => ({...state, data: state.data.concat(appointmentStatuses) }))
    updateAppointmentStatuses = this.updater((state, appointmentStatuses: any[]) => {
        return {
            ...state,
            data: state.data.map((appointmentStatus) => {
            const updated = appointmentStatuses.find((el) => el.id === appointmentStatus.id);
