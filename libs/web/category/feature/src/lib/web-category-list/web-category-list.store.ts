...
import { WebCoreDataAccessService, Category, CorePaging, UserUpdateCategoryInput  } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebCategoryListStore extends ComponentStore<CategoryListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addCategories = this.updater((state, categories: any[]) => ({...state, data: state.data.concat(categories) }))
    updateCategories = this.updater((state, categories: any[]) => {
        return {
            ...state,
            data: state.data.map((category) => {
            const updated = categories.find((el) => el.id === category.id);
