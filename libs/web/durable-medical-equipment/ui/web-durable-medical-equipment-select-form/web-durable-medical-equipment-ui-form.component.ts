...
export class WebFormsUiDurableMedicalEquipmentComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebDurableMedicalEquipmentFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'durableMedicalEquipment_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.durableMedicalEquipment = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'durableMedicalEquipment_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadDurableMedicalEquipmentEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.durableMedicalEquipment = vm.item})
      }
    }
