...
export class WebFormsUiLeadInjuryComponent implements OnInit, OnDestroy {
  @Input() leadInjury: LeadInjury
  ...
  constructor(
    public readonly store: WebLeadInjuryFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'leadInjury_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.leadInjury = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'leadInjury_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadLeadInjuryEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.leadInjury = vm.item})
      }
    }
