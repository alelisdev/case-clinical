...
import { UserCreatePriorAuthorizationRequestInput, WebCoreDataAccessService, PriorAuthorizationRequest, ProcedureSite,SurgicalPosition,User,Document,VisitKind,GuidelineUsed,AuthorizationKind,AuthorizationStatus,Patient,CaseProcedure } from '@case-clinical/web/core/data-access'
...
export interface PriorAuthorizationRequestCreateState {
  errors?: any
  loading?: boolean
...
@Injectable()
export class WebPriorAuthorizationRequestCreateStore extends ComponentStore<PriorAuthorizationRequestCreateState> {
  constructor(
         ...
         this.priorAuthorizationRequestService.createPriorAuthorizationRequest({...input}).pipe(
          tapResponse(
            (priorAuthorizationRequest: PriorAuthorizationRequest) => {
              this.patchState({ item: priorAuthorizationRequest, loading: false })
              return this.router.navigate(['..', priorAuthorizationRequest?.id], {relativeTo: this.route})
            },
            (errors: any) => {
