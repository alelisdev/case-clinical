...
import { WebCoreDataAccessService, PaymentApplicationMethod, CorePaging, UserUpdatePaymentApplicationMethodInput  } from '@case-clinical/web/core/data-access'
...
export interface PaymentApplicationMethodListState {
  headerTitle?: string
  errors?: any
  ...
  paging?: CorePaging
  loading?: boolean
  data?: PaymentApplicationMethod[]
  menuItems?: MenuItem[]
}

@Injectable()
export class WebPaymentApplicationMethodListStore extends ComponentStore<PaymentApplicationMethodListState> {
  constructor(
    ...
    super({
      headerTitle: 'PaymentApplicationMethods',
      searchFocused: false,
      searchQuery: '',
...
readonly vm$ = this.select(
    ...
    addPaymentApplicationMethods = this.updater((state, paymentApplicationMethods: any[]) => ({...state, data: state.data.concat(paymentApplicationMethods) }))
    updatePaymentApplicationMethods = this.updater((state, paymentApplicationMethods: any[]) => {
        return {
            ...state,
            data: state.data.map((paymentApplicationMethod) => {
            const updated = paymentApplicationMethods.find((el) => el.id === paymentApplicationMethod.id);
