...
@Component({
  selector: 'ui-payment-application-method-form',
  template: `
...
export class WebFormsUiPaymentApplicationMethodComponent implements OnInit, OnDestroy {
  @Input() paymentApplicationMethod: PaymentApplicationMethod
  ...
  constructor(
    public readonly store: WebPaymentApplicationMethodFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'paymentApplicationMethod_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.paymentApplicationMethod = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'paymentApplicationMethod_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPaymentApplicationMethodEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.paymentApplicationMethod = vm.item})
      }
    }
