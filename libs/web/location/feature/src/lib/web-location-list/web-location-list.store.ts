...
import { WebCoreDataAccessService, Location, CorePaging, UserUpdateLocationInput  } from '@case-clinical/web/core/data-access'
import { MenuItem } from '@case-clinical/web/ui/dropdown'
import { StackedListItem } from '@case-clinical/web/ui/stacked-list'
import { ComponentStore,tapResponse } from '@ngrx/component-store'
import { map, switchMap, tap, withLatestFrom, catchError } from 'rxjs/operators'
import { ColumnState, SortModelItem } from '@ag-grid-community/core'
import { LocationService } from '@case-clinical/web/location/shared'
...
@Injectable()
export class WebLocationListStore extends ComponentStore<LocationListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addLocations = this.updater((state, locations: any[]) => ({...state, data: state.data.concat(locations) }))
    updateLocations = this.updater((state, locations: any[]) => {
        return {
            ...state,
            data: state.data.map((location) => {
            const updated = locations.find((el) => el.id === location.id);
  ...
  validateImportData(excelData: any[]) {
    return this.vm$.pipe(
      switchMap((vm) => {
        ...
        return this.locationService.validateLocationExcelData(excelData);
      })
    )
  }
