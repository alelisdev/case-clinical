...
@Component({
  selector: 'ui-attorney-status-form',
  template: `
...
export class WebFormsUiAttorneyStatusComponent implements OnInit, OnDestroy {
  @Input() attorneyStatus: AttorneyStatus
  ...
  constructor(
    public readonly store: WebAttorneyStatusFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'attorneyStatus_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.attorneyStatus = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'attorneyStatus_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadAttorneyStatusEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.attorneyStatus = vm.item})
      }
    }
