...
import { WebCoreDataAccessService, AttorneyStatus, CorePaging, UserUpdateAttorneyStatusInput  } from '@case-clinical/web/core/data-access'
...
export interface AttorneyStatusListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebAttorneyStatusListStore extends ComponentStore<AttorneyStatusListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addAttorneyStatuses = this.updater((state, attorneyStatuses: any[]) => ({...state, data: state.data.concat(attorneyStatuses) }))
    updateAttorneyStatuses = this.updater((state, attorneyStatuses: any[]) => {
        return {
            ...state,
            data: state.data.map((attorneyStatus) => {
            const updated = attorneyStatuses.find((el) => el.id === attorneyStatus.id);
