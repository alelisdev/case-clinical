...
export class WebFormsUiCasePreAccidentComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebCasePreAccidentFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'casePreAccident_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.casePreAccident = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'casePreAccident_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadCasePreAccidentEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.casePreAccident = vm.item})
      }
    }
