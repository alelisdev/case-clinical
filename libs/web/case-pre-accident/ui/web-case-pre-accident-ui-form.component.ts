...
import { WebCasePreAccidentFormStore } from './web-case-pre-accident-form.store'
import { CasePreAccident,LegalCase } from '@case-clinical/web/core/data-access'
import { WebUiFormField } from '@case-clinical/web/ui/form'
import { FormlyFieldConfig } from '@ngx-formly/core'
import { map, pluck, tap } from 'rxjs/operators'

@Component({
  selector: 'ui-case-pre-accident-form',
  providers: [WebCasePreAccidentFormStore],
  template: `
    <div class="md:px-2 lg:px-0 lg:col-span-9 dark:bg-gray-800 bg-white rounded-lg shadow p-4">
      <div class="px-6 pt-6">
        <ui-form (submitForm)="submit($any(casePreAccident))" [model]="casePreAccident ?? {}" [fields]="fields" [form]="form">
          <div
...
export class WebFormsUiCasePreAccidentComponent
    {
  @Input() casePreAccident: CasePreAccident = {}
...
WebUiFormField.input('accidentDate', { label: 'Accident Date' }, {className: 'sm:w-full md:w-1/2 lg:w-1/4  px-1'}),
WebUiFormField.input('injuries', { label: 'Injuries' }, {className: 'sm:w-full md:w-1/2 lg:w-1/4  px-1'}),
...
WebUiFormField.checkbox('removed', { label: 'Removed' }, { className: 'w-full sm:w-1/2 md:w-1/4 px-1' })				])
  ...
  constructor(
    private readonly store: WebCasePreAccidentFormStore,
    private readonly route: ActivatedRoute,
    private readonly ref: ChangeDetectorRef
  ) {}


  async submit({ name,legalCaseId,accidentDate,injuries,symptoms,dateCreated,removed }) {
    
    if(this.parentLegalCaseId != ''){
      legalCaseId = this.parentLegalCaseId
    }

    await this.store.createCasePreAccidentEffect({ name,legalCaseId,accidentDate,injuries,symptoms,dateCreated,removed })
