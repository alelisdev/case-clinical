...
@Component({
  selector: 'ui-balance-request-form',
  template: `
...
export class WebFormsUiBalanceRequestComponent implements OnInit, OnDestroy {
  @Input() balanceRequest: BalanceRequest
  ...
  constructor(
    public readonly store: WebBalanceRequestFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'balanceRequest_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.balanceRequest = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'balanceRequest_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadBalanceRequestEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.balanceRequest = vm.item})
      }
    }
