...
import { WebNotificationFormStore } from './web-notification-form.store'
...
@Component({
  ...
  providers: [WebNotificationFormStore],
  template: `
...
WebUiFormField.checkbox('read', { label: 'Read' }, { className: 'w-1/4  p-3' })				])
  ...
  constructor(
    private readonly store: WebNotificationFormStore,
  ) {}

  submit({
name,title,description,type,icon,image,link,useRouter,time,read,userId
  }) {
    this.send.emit({
name,title,description,type,icon,image,link,useRouter,time,read,userId
    })
