...
import { UserUpdateNotificationInput, WebCoreDataAccessService, Notification, User } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebNotificationEditStore extends ComponentStore<NotificationUpdateState> {
...
readonly loadNotificationEffect = this.effect<string>((notificationId$) =>
  ...
  readonly updateNotificationEffect = this.effect<UserUpdateNotificationInput>((input$) =>
    input$.pipe(
      tap(() => this.patchState({ loading: true })),
      withLatestFrom(this.item$),
      switchMap(([input, item]) =>
        this.data.userUpdateNotification({ input, notificationId: item.id }).pipe(
          tapResponse(
            (res) => {
