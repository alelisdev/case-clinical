...
@Component({
  selector: 'ui-procedure-status-form',
  template: `
...
export class WebFormsUiProcedureStatusComponent implements OnInit, OnDestroy {
  @Input() procedureStatus: ProcedureStatus
  ...
  constructor(
    public readonly store: WebProcedureStatusFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'procedureStatus_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.procedureStatus = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'procedureStatus_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadProcedureStatusEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.procedureStatus = vm.item})
      }
    }
