...
import { WebCoreDataAccessService, ProcedureStatus, CorePaging, UserUpdateProcedureStatusInput  } from '@case-clinical/web/core/data-access'
...
export interface ProcedureStatusListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebProcedureStatusListStore extends ComponentStore<ProcedureStatusListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addProcedureStatuses = this.updater((state, procedureStatuses: any[]) => ({...state, data: state.data.concat(procedureStatuses) }))
    updateProcedureStatuses = this.updater((state, procedureStatuses: any[]) => {
        return {
            ...state,
            data: state.data.map((procedureStatus) => {
            const updated = procedureStatuses.find((el) => el.id === procedureStatus.id);
