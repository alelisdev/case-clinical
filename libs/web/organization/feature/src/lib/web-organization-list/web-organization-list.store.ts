...
import { WebCoreDataAccessService, Organization, CorePaging, UserUpdateOrganizationInput  } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebOrganizationListStore extends ComponentStore<OrganizationListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addOrganizations = this.updater((state, organizations: any[]) => ({...state, data: state.data.concat(organizations) }))
    updateOrganizations = this.updater((state, organizations: any[]) => {
        return {
            ...state,
            data: state.data.map((organization) => {
            const updated = organizations.find((el) => el.id === organization.id);
