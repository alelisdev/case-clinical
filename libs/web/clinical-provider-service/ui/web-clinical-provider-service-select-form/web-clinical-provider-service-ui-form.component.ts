...
@Component({
  selector: 'ui-clinical-provider-service-form',
  template: `
...
export class WebFormsUiClinicalProviderServiceComponent implements OnInit, OnDestroy {
  @Input() clinicalProviderService: ClinicalProviderService
  ...
  constructor(
    public readonly store: WebClinicalProviderServiceFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'clinicalProviderService_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.clinicalProviderService = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'clinicalProviderService_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadClinicalProviderServiceEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.clinicalProviderService = vm.item})
      }
    }
