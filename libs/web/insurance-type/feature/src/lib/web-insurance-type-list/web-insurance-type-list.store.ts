...
import { WebCoreDataAccessService, InsuranceType, CorePaging, UserUpdateInsuranceTypeInput  } from '@case-clinical/web/core/data-access'
...
export interface InsuranceTypeListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebInsuranceTypeListStore extends ComponentStore<InsuranceTypeListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addInsuranceTypes = this.updater((state, insuranceTypes: any[]) => ({...state, data: state.data.concat(insuranceTypes) }))
    updateInsuranceTypes = this.updater((state, insuranceTypes: any[]) => {
        return {
            ...state,
            data: state.data.map((insuranceType) => {
            const updated = insuranceTypes.find((el) => el.id === insuranceType.id);
