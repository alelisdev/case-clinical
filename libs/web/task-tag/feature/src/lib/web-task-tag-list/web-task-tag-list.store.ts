...
import { WebCoreDataAccessService, TaskTag, CorePaging } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebTaskTagListStore extends ComponentStore<TaskTagListState> {
  constructor(private readonly data: WebCoreDataAccessService) {
    super({
      headerTitle: 'TaskTags',
      searchFocused: false,
      searchQuery: '',
      paging: {
        limit: 10000,
  ...
  private mapDataToItems(data: TaskTag[]) {
    return (data || []).map(
      ({ name, id, createdAt }) =>
        ({
          title: name,
