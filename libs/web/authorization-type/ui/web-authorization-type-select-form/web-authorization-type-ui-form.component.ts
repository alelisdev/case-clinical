...
@Component({
  selector: 'ui-authorization-type-form',
  template: `
...
export class WebFormsUiAuthorizationTypeComponent implements OnInit, OnDestroy {
  @Input() authorizationType: AuthorizationType
  ...
  constructor(
    public readonly store: WebAuthorizationTypeFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'authorizationType_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.authorizationType = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'authorizationType_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadAuthorizationTypeEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.authorizationType = vm.item})
      }
    }
