...
export class WebFormsUiCaseAccountComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebCaseAccountFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'caseAccount_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: ''})
      this.store.vm$.subscribe((vm) => {
      this.caseAccount = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'caseAccount_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadCaseAccountEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.caseAccount = vm.item})
      }
    }
