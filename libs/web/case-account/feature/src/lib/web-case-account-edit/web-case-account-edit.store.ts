...
import { UserUpdateCaseAccountInput, WebCoreDataAccessService, CaseAccount, LegalCase,Location,Vendor,AccountStatus,ProcedureType,AgreementType,User,Contract,Portfolio,ProcedureVendor } from '@case-clinical/web/core/data-access'
...
export interface CaseAccountEditState {
  errors?: any
  loading?: boolean
  item?: CaseAccount,
...
@Injectable()
export class WebCaseAccountEditStore extends ComponentStore<CaseAccountEditState> {
  constructor(
    ...
    this.loadCaseAccountEffect(route.params.pipe(map((route) => route?.caseAccountId)))
  }
  ...
  readonly updateCaseAccountEffect = this.effect<UserUpdateCaseAccountInput>((input$) =>
    input$.pipe(
      ...
      switchMap(([input, item]) =>
         this.caseAccountService.updateCaseAccount(input, item?.id).pipe(
          tapResponse(
            (response: any) => {
