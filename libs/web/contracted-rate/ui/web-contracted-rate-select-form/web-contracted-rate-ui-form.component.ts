...
export class WebFormsUiContractedRateComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebContractedRateFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contractedRate_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.contractedRate = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contractedRate_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadContractedRateEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.contractedRate = vm.item})
      }
    }
