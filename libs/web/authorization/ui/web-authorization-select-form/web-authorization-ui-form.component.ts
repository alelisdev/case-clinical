...
@Component({
  selector: 'ui-authorization-form',
  template: `
...
export class WebFormsUiAuthorizationComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebAuthorizationFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'authorization_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.authorization = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'authorization_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadAuthorizationEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.authorization = vm.item})
      }
    }
