...
import { WebCoreDataAccessService, Role, CorePaging, UserUpdateRoleInput  } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebRoleListStore extends ComponentStore<RoleListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addRoles = this.updater((state, roles: any[]) => ({...state, data: state.data.concat(roles) }))
    updateRoles = this.updater((state, roles: any[]) => {
        return {
            ...state,
            data: state.data.map((role) => {
            const updated = roles.find((el) => el.id === role.id);
