...
export class WebFormsUiRoleComponent implements OnInit, OnDestroy {
  @Input() role: Role
  ...
  constructor(
    public readonly store: WebRoleFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'role_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.role = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'role_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadRoleEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.role = vm.item})
      }
    }
