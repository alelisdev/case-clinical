...
export class WebFormsUiTeamComponent implements OnInit, OnDestroy {
  @Input() team: Team
  ...
  constructor(
    public readonly store: WebTeamFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'team_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.team = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'team_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadTeamEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.team = vm.item})
      }
    }
