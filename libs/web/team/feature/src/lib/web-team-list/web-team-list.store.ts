...
import { WebCoreDataAccessService, Team, CorePaging, UserUpdateTeamInput  } from '@case-clinical/web/core/data-access'
import { MenuItem } from '@case-clinical/web/ui/dropdown'
import { StackedListItem } from '@case-clinical/web/ui/stacked-list'
import { ComponentStore,tapResponse } from '@ngrx/component-store'
import { map, switchMap, tap, withLatestFrom, catchError } from 'rxjs/operators'
import { ColumnState, SortModelItem } from '@ag-grid-community/core'
import { TeamService } from '@case-clinical/web/team/shared'
...
@Injectable()
export class WebTeamListStore extends ComponentStore<TeamListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addTeams = this.updater((state, teams: any[]) => ({...state, data: state.data.concat(teams) }))
    updateTeams = this.updater((state, teams: any[]) => {
        return {
            ...state,
            data: state.data.map((team) => {
            const updated = teams.find((el) => el.id === team.id);
