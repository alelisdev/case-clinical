

import { Injectable } from '@angular/core'
import { ActivatedRoute } from '@angular/router'
import { WebCoreDataAccessService, TeamUser, CorePaging, UserUpdateTeamUserInput ,Team, User, TeamRole } from '@case-clinical/web/core/data-access'
import { MenuItem } from '@case-clinical/web/ui/dropdown'
import { StackedListItem } from '@case-clinical/web/ui/stacked-list'
import { ComponentStore,tapResponse } from '@ngrx/component-store'
import { map, switchMap, tap, withLatestFrom, catchError } from 'rxjs/operators'
import { ColumnState, SortModelItem } from '@ag-grid-community/core'
import { TeamUserService } from '@case-clinical/web/team-user/shared'
import { EMPTY } from 'rxjs'
import { WebUiToastService } from '@case-clinical/web/ui/toast'
import { WebTeamFeatureStore } from '@case-clinical/web/team/shared'
import { WebUserFeatureStore } from '@case-clinical/web/user/shared'
import { WebTeamRoleFeatureStore } from '@case-clinical/web/team-role/shared'

export interface FilterState {
  [key: string]: unknown;
}

export interface TeamUserListState {
  headerTitle?: string
  errors?: any
  searchFocused: boolean
  searchQuery?: string
teamId?: string,userId?: string,teamRoleId?: string,
  sortSettings: ColumnState[]
  filterSettings: FilterState
  paging?: CorePaging
  loading?: boolean
  data?: TeamUser[]
  menuItems?: MenuItem[]
}

@Injectable()
export class WebTeamUserListStore extends ComponentStore<TeamUserListState> {
  constructor(
        private readonly data: WebCoreDataAccessService, 
        private readonly router: ActivatedRoute,
        private readonly teamUserService: TeamUserService,
        private readonly toast: WebUiToastService,
         private readonly teamStore: WebTeamFeatureStore,
 private readonly userStore: WebUserFeatureStore,
 private readonly teamRoleStore: WebTeamRoleFeatureStore
    ) {
    super({
      headerTitle: 'TeamUsers',
      searchFocused: false,
      searchQuery: '',
teamId: undefined,
userId: undefined,
teamRoleId: undefined,
      sortSettings: [],
      filterSettings: {},
      paging: {
        limit: 10000,
        skip: 0,
      },
    })

    
    if(this.router.snapshot.paramMap.has("teamId")) {
      var teamId = this.router.snapshot.paramMap.get("teamId")
      this.setTeamId(teamId)
    }


    if(this.router.snapshot.paramMap.has("userId")) {
      var userId = this.router.snapshot.paramMap.get("userId")
      this.setUserId(userId)
    }


    if(this.router.snapshot.paramMap.has("teamRoleId")) {
      var teamRoleId = this.router.snapshot.paramMap.get("teamRoleId")
      this.setTeamRoleId(teamRoleId)
    }

    this.teamStore.loadTeamsEffect()
this.userStore.loadUsersEffect()
this.teamRoleStore.loadTeamRolesEffect()
  }

  readonly setSkip = this.updater((state, skip: number) => ({
    ...state,
    paging: { ...state.paging, skip }
  }))

  readonly setSearchQuery = this.updater((state, searchQuery: string) => ({
    ...state,
    searchQuery
  }))

  readonly setSearchBarInFocus = this.updater((state, searchFocused: boolean) => ({
    ...state,
    searchFocused
  }))


            readonly setTeamId = this.updater((state, teamId: string) => ({
                ...state,
    teamId,
  }))


            readonly setUserId = this.updater((state, userId: string) => ({
                ...state,
    userId,
  }))


            readonly setTeamRoleId = this.updater((state, teamRoleId: string) => ({
                ...state,
    teamRoleId,
  }))


  readonly headerTitle$ = this.select((s) => s.headerTitle)
  readonly errors$ = this.select((s) => s.errors)
  readonly loading$ = this.select((s) => s.loading)
  readonly paging$ = this.select((s) => s.paging)
  readonly searchFocused$ = this.select((s) => s.searchFocused)
  readonly searchQuery$ = this.select((s) => s.searchQuery)

readonly teamId$ = this.select((s) => s.teamId)

readonly userId$ = this.select((s) => s.userId)

readonly teamRoleId$ = this.select((s) => s.teamRoleId)

teams$ = this.teamStore.teams$
users$ = this.userStore.users$
teamRoles$ = this.teamRoleStore.teamRoles$
  readonly data$ = this.select((s) => s.data)

  readonly sortSettings$ = this.select((s) => s.sortSettings)
  readonly filterSettings$ = this.select((s) => s.filterSettings)

  readonly input$ = this.select(this.paging$, this.teamId$,
this.userId$,
this.teamRoleId$, this.searchQuery$, (paging, teamId,
userId,
teamRoleId,searchQuery) => ({
    limit: paging.limit,
    skip: paging.skip,
    name: searchQuery,
    teamId: teamId,userId: userId,teamRoleId: teamRoleId,
    total: paging.total
  }))

readonly vm$ = this.select(
    this.paging$,
    this.errors$,
    this.loading$,
    this.searchFocused$,
    this.searchQuery$,
    this.teamId$,
this.userId$,
this.teamRoleId$,
    this.data$,
    this.teams$,
this.users$,
this.teamRoles$,
    (paging, errors, loading, searchFocused, searchQuery, teamId,
userId,
teamRoleId, data ,teams,users,teamRoles) => ({
      paging,
      errors,
      loading,
      searchFocused,
      searchQuery,
      teamId,
userId,
teamRoleId,
      data,
      teams,users,teamRoles
    }),
  )

    addTeamUsers = this.updater((state, teamUsers: any[]) => ({...state, data: state.data.concat(teamUsers) }))
    updateTeamUsers = this.updater((state, teamUsers: any[]) => {
        return {
            ...state,
            data: state.data.map((teamUser) => {
            const updated = teamUsers.find((el) => el.id === teamUser.id);
            return updated ? updated : teamUser;
            })
        }
    })

  validateImportData(excelData: any[]) {
    return this.vm$.pipe(
      switchMap((vm) => {
        const teams = vm.teams;
const users = vm.users;
const teamRoles = vm.teamRoles;
        return this.teamUserService.validateTeamUserExcelData(excelData,teams,users,teamRoles);
      })
    )
  }


  readonly loadTeamUsersEffect = this.effect(($) =>
    $.pipe(
      tap(() => this.patchState({ loading: true })),
      withLatestFrom(this.input$),
      switchMap(([_, input]) =>
        this.data.userTeamUsers({input}).pipe(
          tapResponse(
            (res) =>
              this.patchState({
                paging: {limit: input.limit, skip: input.skip, total: res.data.count.total},
                data: res.data.items,
                errors: res.errors,
                loading: false,
              }),
            (errors: any) =>
              this.patchState({
                loading: false,
                errors: errors.graphQLErrors ? errors.graphQLErrors : errors,
              }),
          ),
        ),
      ),
    ),
  )

readonly importExcelEffect = this.effect<UserUpdateTeamUserInput[]>(($data) =>
    $data.pipe(
      tap(() => this.patchState({loading: true })),
      switchMap((data) => this.teamUserService.importTeamUsers(data).pipe(
        catchError(error => {
          console.log(error)
          this.toast.error(error.Message ?? 'Failed to save', {duration: 3000 })
          return EMPTY;
        }),
        tap(
          (updateResult) => {
            const created = JSON.parse(updateResult.created);
            const updated = JSON.parse(updateResult.updated);
            const failed = JSON.parse(updateResult.failed);
            const total = created.length + updated.length + failed.length;

            this.addTeamUsers(created);
            this.updateTeamUsers(updated);

            this.toast.success(`${created.length} created, ${updated.length} updated, ${failed.length} failed of total ${total}`, { duration: 3000 })
          }
        )
      ))
    )
  )
}

