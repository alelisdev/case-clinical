...
export class WebFormsUiTeamUserComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebTeamUserFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'teamUser_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.teamUser = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'teamUser_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadTeamUserEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.teamUser = vm.item})
      }
    }
