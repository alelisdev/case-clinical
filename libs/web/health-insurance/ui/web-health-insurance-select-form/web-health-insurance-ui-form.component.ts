...
export class WebFormsUiHealthInsuranceComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebHealthInsuranceFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'healthInsurance_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.healthInsurance = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'healthInsurance_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadHealthInsuranceEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.healthInsurance = vm.item})
      }
    }
