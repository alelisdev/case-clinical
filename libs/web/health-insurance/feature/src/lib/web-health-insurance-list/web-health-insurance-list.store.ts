...
import { WebCoreDataAccessService, HealthInsurance, CorePaging, UserUpdateHealthInsuranceInput  } from '@case-clinical/web/core/data-access'
...
export interface HealthInsuranceListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebHealthInsuranceListStore extends ComponentStore<HealthInsuranceListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addHealthInsurances = this.updater((state, healthInsurances: any[]) => ({...state, data: state.data.concat(healthInsurances) }))
    updateHealthInsurances = this.updater((state, healthInsurances: any[]) => {
        return {
            ...state,
            data: state.data.map((healthInsurance) => {
            const updated = healthInsurances.find((el) => el.id === healthInsurance.id);
