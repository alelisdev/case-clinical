...
import { Permission } from '@case-clinical/shared/util/sdk'
export class ValidateRolePermissionExcelDataAction extends RolePermissionBusinessActionBase<boolean> {
  ...
  constructor(excelData: any[], permissions: Permission[]) {
    ...
    this.excelData.map((datum, index) => {
      names.push(datum['name']);
            ...
            this.validationContext.addRule(
              new ImportSolutionRule(`permissionName_${index}_is_valid}`, "Permission Is Not Valid", 'permission.name', datum['permission'], this.permissions, true)
            )
    })
