...
@Component({
  selector: 'ui-guideline-used-form',
  template: `
...
export class WebFormsUiGuidelineUsedComponent implements OnInit, OnDestroy {
  @Input() guidelineUsed: GuidelineUsed
  ...
  constructor(
    public readonly store: WebGuidelineUsedFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'guidelineUsed_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.guidelineUsed = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'guidelineUsed_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadGuidelineUsedEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.guidelineUsed = vm.item})
      }
    }
