...
import { WebCoreDataAccessService, GuidelineUsed, CorePaging, UserUpdateGuidelineUsedInput  } from '@case-clinical/web/core/data-access'
...
export interface GuidelineUsedListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebGuidelineUsedListStore extends ComponentStore<GuidelineUsedListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addGuidelineUseds = this.updater((state, guidelineUseds: any[]) => ({...state, data: state.data.concat(guidelineUseds) }))
    updateGuidelineUseds = this.updater((state, guidelineUseds: any[]) => {
        return {
            ...state,
            data: state.data.map((guidelineUsed) => {
            const updated = guidelineUseds.find((el) => el.id === guidelineUsed.id);
