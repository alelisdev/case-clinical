...
export class WebFormsUiAwardComponent implements OnInit, OnDestroy {
  @Input() award: Award
  ...
  constructor(
    public readonly store: WebAwardFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'award_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.award = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'award_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadAwardEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.award = vm.item})
      }
    }
