...
@Component({
  selector: 'ui-attorney-type-form',
  template: `
...
export class WebFormsUiAttorneyTypeComponent implements OnInit, OnDestroy {
  @Input() attorneyType: AttorneyType
  ...
  constructor(
    public readonly store: WebAttorneyTypeFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'attorneyType_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.attorneyType = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'attorneyType_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadAttorneyTypeEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.attorneyType = vm.item})
      }
    }
