...
import { WebCoreDataAccessService, AttorneyType, CorePaging, UserUpdateAttorneyTypeInput  } from '@case-clinical/web/core/data-access'
...
export interface AttorneyTypeListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebAttorneyTypeListStore extends ComponentStore<AttorneyTypeListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addAttorneyTypes = this.updater((state, attorneyTypes: any[]) => ({...state, data: state.data.concat(attorneyTypes) }))
    updateAttorneyTypes = this.updater((state, attorneyTypes: any[]) => {
        return {
            ...state,
            data: state.data.map((attorneyType) => {
            const updated = attorneyTypes.find((el) => el.id === attorneyType.id);
