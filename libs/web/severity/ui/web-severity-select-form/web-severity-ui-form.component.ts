...
export class WebFormsUiSeverityComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebSeverityFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'severity_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.severity = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'severity_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadSeverityEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.severity = vm.item})
      }
    }
