...
@Component({
  selector: 'ui-authorization-diagnosis-code-form',
  template: `
...
export class WebFormsUiAuthorizationDiagnosisCodeComponent implements OnInit, OnDestroy {
  @Input() authorizationDiagnosisCode: AuthorizationDiagnosisCode
  ...
  constructor(
    public readonly store: WebAuthorizationDiagnosisCodeFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'authorizationDiagnosisCode_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.authorizationDiagnosisCode = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'authorizationDiagnosisCode_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadAuthorizationDiagnosisCodeEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.authorizationDiagnosisCode = vm.item})
      }
    }
