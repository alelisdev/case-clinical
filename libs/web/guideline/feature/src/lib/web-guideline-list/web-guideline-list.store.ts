...
import { WebCoreDataAccessService, Guideline, CorePaging, UserUpdateGuidelineInput  } from '@case-clinical/web/core/data-access'
...
export interface GuidelineListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebGuidelineListStore extends ComponentStore<GuidelineListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addGuidelines = this.updater((state, guidelines: any[]) => ({...state, data: state.data.concat(guidelines) }))
    updateGuidelines = this.updater((state, guidelines: any[]) => {
        return {
            ...state,
            data: state.data.map((guideline) => {
            const updated = guidelines.find((el) => el.id === guideline.id);
