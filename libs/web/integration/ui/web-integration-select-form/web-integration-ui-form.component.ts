...
export class WebFormsUiIntegrationComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebIntegrationFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'integration_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.integration = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'integration_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadIntegrationEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.integration = vm.item})
      }
    }
