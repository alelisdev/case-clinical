...
@Component({
  selector: 'ui-clinical-provider-tag-form',
  template: `
...
export class WebFormsUiClinicalProviderTagComponent implements OnInit, OnDestroy {
  @Input() clinicalProviderTag: ClinicalProviderTag
  ...
  constructor(
    public readonly store: WebClinicalProviderTagFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'clinicalProviderTag_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.clinicalProviderTag = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'clinicalProviderTag_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadClinicalProviderTagEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.clinicalProviderTag = vm.item})
      }
    }
