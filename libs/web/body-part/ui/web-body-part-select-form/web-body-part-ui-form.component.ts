...
export class WebFormsUiBodyPartComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebBodyPartFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'bodyPart_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.bodyPart = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'bodyPart_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadBodyPartEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.bodyPart = vm.item})
      }
    }
