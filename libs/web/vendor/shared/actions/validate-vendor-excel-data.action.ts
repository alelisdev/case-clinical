...
import { VendorType } from '@case-clinical/shared/util/sdk'
export class ValidateVendorExcelDataAction extends VendorBusinessActionBase<boolean> {
  ...
  constructor(excelData: any[], vendorTypes: VendorType[]) {
    ...
    this.excelData.map((datum, index) => {
      names.push(datum['name']);
            ...
            this.validationContext.addRule(
              new ImportSolutionRule(`vendorTypeName_${index}_is_valid}`, "Vendor Type Is Not Valid", 'vendorType.name', datum['vendorType'], this.vendorTypes, true)
            )
    })
