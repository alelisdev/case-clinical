...
export class WebFormsUiVendorComponent implements OnInit, OnDestroy {
  @Input() vendor: Vendor
  ...
  constructor(
    public readonly store: WebVendorFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'vendor_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.vendor = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'vendor_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadVendorEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.vendor = vm.item})
      }
    }
