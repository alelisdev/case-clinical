...
import { WebCoreDataAccessService, UserCalendar, CorePaging } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebUserCalendarListStore extends ComponentStore<UserCalendarListState> {
  constructor(private readonly data: WebCoreDataAccessService) {
    super({
      headerTitle: 'UserCalendars',
      searchFocused: false,
      searchQuery: '',
      paging: {
        limit: 10000,
  ...
  private mapDataToItems(data: UserCalendar[]) {
    return (data || []).map(
      ({ name, id, createdAt }) =>
        ({
          title: name,
