...
import { WebCoreDataAccessService, Portfolio, CorePaging, UserUpdatePortfolioInput  } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebPortfolioListStore extends ComponentStore<PortfolioListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addPortfolios = this.updater((state, portfolios: any[]) => ({...state, data: state.data.concat(portfolios) }))
    updatePortfolios = this.updater((state, portfolios: any[]) => {
        return {
            ...state,
            data: state.data.map((portfolio) => {
            const updated = portfolios.find((el) => el.id === portfolio.id);
