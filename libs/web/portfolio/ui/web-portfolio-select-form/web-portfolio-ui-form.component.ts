...
export class WebFormsUiPortfolioComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebPortfolioFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'portfolio_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.portfolio = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'portfolio_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPortfolioEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.portfolio = vm.item})
      }
    }
