...
export class WebFormsUiContractedRateKindComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebContractedRateKindFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contractedRateKind_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.contractedRateKind = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contractedRateKind_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadContractedRateKindEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.contractedRateKind = vm.item})
      }
    }
