...
import { WebCoreDataAccessService, ContractedRateKind, CorePaging, UserUpdateContractedRateKindInput  } from '@case-clinical/web/core/data-access'
...
export interface ContractedRateKindListState {
  headerTitle?: string
  errors?: any
  ...
  paging?: CorePaging
  loading?: boolean
  data?: ContractedRateKind[]
  menuItems?: MenuItem[]
}

@Injectable()
export class WebContractedRateKindListStore extends ComponentStore<ContractedRateKindListState> {
  constructor(
    ...
    super({
      headerTitle: 'ContractedRateKinds',
      searchFocused: false,
      searchQuery: '',
...
readonly vm$ = this.select(
    ...
    addContractedRateKinds = this.updater((state, contractedRateKinds: any[]) => ({...state, data: state.data.concat(contractedRateKinds) }))
    updateContractedRateKinds = this.updater((state, contractedRateKinds: any[]) => {
        return {
            ...state,
            data: state.data.map((contractedRateKind) => {
            const updated = contractedRateKinds.find((el) => el.id === contractedRateKind.id);
