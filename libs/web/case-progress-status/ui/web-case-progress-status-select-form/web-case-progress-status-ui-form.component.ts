...
@Component({
  selector: 'ui-case-progress-status-form',
  template: `
...
export class WebFormsUiCaseProgressStatusComponent implements OnInit, OnDestroy {
  @Input() caseProgressStatus: CaseProgressStatus
  ...
  constructor(
    public readonly store: WebCaseProgressStatusFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'caseProgressStatus_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.caseProgressStatus = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'caseProgressStatus_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadCaseProgressStatusEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.caseProgressStatus = vm.item})
      }
    }
