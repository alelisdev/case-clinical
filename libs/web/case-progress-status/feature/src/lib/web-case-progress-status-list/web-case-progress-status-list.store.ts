...
import { WebCoreDataAccessService, CaseProgressStatus, CorePaging, UserUpdateCaseProgressStatusInput  } from '@case-clinical/web/core/data-access'
...
export interface CaseProgressStatusListState {
  headerTitle?: string
  errors?: any
  ...
  paging?: CorePaging
  loading?: boolean
  data?: CaseProgressStatus[]
  menuItems?: MenuItem[]
}

@Injectable()
export class WebCaseProgressStatusListStore extends ComponentStore<CaseProgressStatusListState> {
  constructor(
    ...
    super({
      headerTitle: 'CaseProgressStatuses',
      searchFocused: false,
      searchQuery: '',
...
readonly vm$ = this.select(
    ...
    addCaseProgressStatuses = this.updater((state, caseProgressStatuses: any[]) => ({...state, data: state.data.concat(caseProgressStatuses) }))
    updateCaseProgressStatuses = this.updater((state, caseProgressStatuses: any[]) => {
        return {
            ...state,
            data: state.data.map((caseProgressStatus) => {
            const updated = caseProgressStatuses.find((el) => el.id === caseProgressStatus.id);
