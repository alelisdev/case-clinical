...
export class WebFormsUiPriorAuthorizationDiagnosisCodeComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebPriorAuthorizationDiagnosisCodeFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'priorAuthorizationDiagnosisCode_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.priorAuthorizationDiagnosisCode = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'priorAuthorizationDiagnosisCode_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPriorAuthorizationDiagnosisCodeEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.priorAuthorizationDiagnosisCode = vm.item})
      }
    }
