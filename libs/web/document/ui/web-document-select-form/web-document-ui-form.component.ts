...
export class WebFormsUiDocumentComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebDocumentFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'document_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.document = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'document_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadDocumentEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.document = vm.item})
      }
    }
