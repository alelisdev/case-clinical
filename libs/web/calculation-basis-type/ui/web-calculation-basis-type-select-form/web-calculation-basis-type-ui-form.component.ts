...
@Component({
  selector: 'ui-calculation-basis-type-form',
  template: `
...
export class WebFormsUiCalculationBasisTypeComponent implements OnInit, OnDestroy {
  @Input() calculationBasisType: CalculationBasisType
  ...
  constructor(
    public readonly store: WebCalculationBasisTypeFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'calculationBasisType_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.calculationBasisType = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'calculationBasisType_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadCalculationBasisTypeEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.calculationBasisType = vm.item})
      }
    }
