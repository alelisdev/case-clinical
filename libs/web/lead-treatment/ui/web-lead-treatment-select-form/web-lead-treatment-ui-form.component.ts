...
@Component({
  selector: 'ui-lead-treatment-form',
  template: `
...
export class WebFormsUiLeadTreatmentComponent implements OnInit, OnDestroy {
  @Input() leadTreatment: LeadTreatment
  ...
  constructor(
    public readonly store: WebLeadTreatmentFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'leadTreatment_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.leadTreatment = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'leadTreatment_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadLeadTreatmentEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.leadTreatment = vm.item})
      }
    }
