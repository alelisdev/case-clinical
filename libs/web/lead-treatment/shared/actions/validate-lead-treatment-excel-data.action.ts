...
import { Lead, Treatment } from '@case-clinical/shared/util/sdk'
export class ValidateLeadTreatmentExcelDataAction extends LeadTreatmentBusinessActionBase<boolean> {
...
treatments: Treatment[]
  ...
  constructor(excelData: any[], leads: Lead[], treatments: Treatment[]) {
  ...
  preValidateAction() {
    ...
    this.excelData.map((datum, index) => {
      names.push(datum['name']);
            ...
            this.validationContext.addRule(
              new ImportSolutionRule(`treatmentName_${index}_is_valid}`, "Treatment Is Not Valid", 'treatment.name', datum['treatment'], this.treatments, true)
            )
    })
