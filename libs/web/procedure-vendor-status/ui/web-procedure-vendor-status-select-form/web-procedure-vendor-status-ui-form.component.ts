...
@Component({
  selector: 'ui-procedure-vendor-status-form',
  template: `
...
export class WebFormsUiProcedureVendorStatusComponent implements OnInit, OnDestroy {
  @Input() procedureVendorStatus: ProcedureVendorStatus
  ...
  constructor(
    public readonly store: WebProcedureVendorStatusFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'procedureVendorStatus_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({name: ''})
      this.store.vm$.subscribe((vm) => {
      this.procedureVendorStatus = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'procedureVendorStatus_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadProcedureVendorStatusEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.procedureVendorStatus = vm.item})
      }
    }
