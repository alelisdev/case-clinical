...
import { WebCoreDataAccessService, PlaceOfService, CorePaging, UserUpdatePlaceOfServiceInput  } from '@case-clinical/web/core/data-access'
...
export interface PlaceOfServiceListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebPlaceOfServiceListStore extends ComponentStore<PlaceOfServiceListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addPlaceOfServices = this.updater((state, placeOfServices: any[]) => ({...state, data: state.data.concat(placeOfServices) }))
    updatePlaceOfServices = this.updater((state, placeOfServices: any[]) => {
        return {
            ...state,
            data: state.data.map((placeOfService) => {
            const updated = placeOfServices.find((el) => el.id === placeOfService.id);
