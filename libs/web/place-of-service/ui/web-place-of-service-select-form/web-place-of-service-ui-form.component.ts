...
export class WebFormsUiPlaceOfServiceComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebPlaceOfServiceFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'placeOfService_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.placeOfService = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'placeOfService_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPlaceOfServiceEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.placeOfService = vm.item})
      }
    }
