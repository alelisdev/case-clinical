...
import { WebCoreDataAccessService, VisitKind, CorePaging, UserUpdateVisitKindInput  } from '@case-clinical/web/core/data-access'
...
export interface VisitKindListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebVisitKindListStore extends ComponentStore<VisitKindListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addVisitKinds = this.updater((state, visitKinds: any[]) => ({...state, data: state.data.concat(visitKinds) }))
    updateVisitKinds = this.updater((state, visitKinds: any[]) => {
        return {
            ...state,
            data: state.data.map((visitKind) => {
            const updated = visitKinds.find((el) => el.id === visitKind.id);
