...
export class WebFormsUiVisitKindComponent implements OnInit, OnDestroy {
  @Input() visitKind: VisitKind
  ...
  constructor(
    public readonly store: WebVisitKindFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'visitKind_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.visitKind = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'visitKind_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadVisitKindEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.visitKind = vm.item})
      }
    }
