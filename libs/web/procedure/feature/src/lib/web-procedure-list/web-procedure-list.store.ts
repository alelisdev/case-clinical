...
import { WebCoreDataAccessService, Procedure, CorePaging, UserUpdateProcedureInput  } from '@case-clinical/web/core/data-access'
...
export interface ProcedureListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebProcedureListStore extends ComponentStore<ProcedureListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addProcedures = this.updater((state, procedures: any[]) => ({...state, data: state.data.concat(procedures) }))
    updateProcedures = this.updater((state, procedures: any[]) => {
        return {
            ...state,
            data: state.data.map((procedure) => {
            const updated = procedures.find((el) => el.id === procedure.id);
