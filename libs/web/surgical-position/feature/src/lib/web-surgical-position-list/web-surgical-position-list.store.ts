...
import { WebCoreDataAccessService, SurgicalPosition, CorePaging, UserUpdateSurgicalPositionInput  } from '@case-clinical/web/core/data-access'
...
export interface SurgicalPositionListState {
  headerTitle?: string
  errors?: any
  ...
  paging?: CorePaging
  loading?: boolean
  data?: SurgicalPosition[]
  menuItems?: MenuItem[]
}

@Injectable()
export class WebSurgicalPositionListStore extends ComponentStore<SurgicalPositionListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addSurgicalPositions = this.updater((state, surgicalPositions: any[]) => ({...state, data: state.data.concat(surgicalPositions) }))
    updateSurgicalPositions = this.updater((state, surgicalPositions: any[]) => {
        return {
            ...state,
            data: state.data.map((surgicalPosition) => {
            const updated = surgicalPositions.find((el) => el.id === surgicalPosition.id);
