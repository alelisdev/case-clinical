...
@Component({
  selector: 'ui-surgical-position-form',
  template: `
...
export class WebFormsUiSurgicalPositionComponent implements OnInit, OnDestroy {
  @Input() surgicalPosition: SurgicalPosition
  ...
  constructor(
    public readonly store: WebSurgicalPositionFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'surgicalPosition_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.surgicalPosition = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'surgicalPosition_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadSurgicalPositionEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.surgicalPosition = vm.item})
      }
    }
