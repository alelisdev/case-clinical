...
import { WebCoreDataAccessService, Bank, CorePaging, UserUpdateBankInput  } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebBankListStore extends ComponentStore<BankListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addBanks = this.updater((state, banks: any[]) => ({...state, data: state.data.concat(banks) }))
    updateBanks = this.updater((state, banks: any[]) => {
        return {
            ...state,
            data: state.data.map((bank) => {
            const updated = banks.find((el) => el.id === bank.id);
