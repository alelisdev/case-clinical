...
export class WebFormsUiBankComponent implements OnInit, OnDestroy {
  @Input() bank: Bank
  ...
  constructor(
    public readonly store: WebBankFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'bank_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.bank = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'bank_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadBankEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.bank = vm.item})
      }
    }
