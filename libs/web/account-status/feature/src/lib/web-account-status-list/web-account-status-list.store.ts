...
import { WebCoreDataAccessService, AccountStatus, CorePaging, UserUpdateAccountStatusInput  } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebAccountStatusListStore extends ComponentStore<AccountStatusListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addAccountStatuses = this.updater((state, accountStatuses: any[]) => ({...state, data: state.data.concat(accountStatuses) }))
    updateAccountStatuses = this.updater((state, accountStatuses: any[]) => {
        return {
            ...state,
            data: state.data.map((accountStatus) => {
            const updated = accountStatuses.find((el) => el.id === accountStatus.id);
