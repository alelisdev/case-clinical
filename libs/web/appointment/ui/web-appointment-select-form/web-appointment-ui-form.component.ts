...
export class WebFormsUiAppointmentComponent implements OnInit, OnDestroy {
  ...
  constructor(
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'appointment_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.appointment = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'appointment_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadAppointmentEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.appointment = vm.item})
      }
    }
