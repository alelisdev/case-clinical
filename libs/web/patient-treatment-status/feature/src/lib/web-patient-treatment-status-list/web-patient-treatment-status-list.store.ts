...
import { WebCoreDataAccessService, PatientTreatmentStatus, CorePaging, UserUpdatePatientTreatmentStatusInput  } from '@case-clinical/web/core/data-access'
...
export interface PatientTreatmentStatusListState {
  headerTitle?: string
  errors?: any
  ...
  paging?: CorePaging
  loading?: boolean
  data?: PatientTreatmentStatus[]
  menuItems?: MenuItem[]
}

@Injectable()
export class WebPatientTreatmentStatusListStore extends ComponentStore<PatientTreatmentStatusListState> {
  constructor(
    ...
    super({
      headerTitle: 'PatientTreatmentStatuses',
      searchFocused: false,
      searchQuery: '',
...
readonly vm$ = this.select(
    ...
    addPatientTreatmentStatuses = this.updater((state, patientTreatmentStatuses: any[]) => ({...state, data: state.data.concat(patientTreatmentStatuses) }))
    updatePatientTreatmentStatuses = this.updater((state, patientTreatmentStatuses: any[]) => {
        return {
            ...state,
            data: state.data.map((patientTreatmentStatus) => {
            const updated = patientTreatmentStatuses.find((el) => el.id === patientTreatmentStatus.id);
