...
@Component({
  selector: 'ui-prior-meds-to-date-status-form',
  template: `
...
export class WebFormsUiPriorMedsToDateStatusComponent implements OnInit, OnDestroy {
  @Input() priorMedsToDateStatus: PriorMedsToDateStatus
  ...
  constructor(
    public readonly store: WebPriorMedsToDateStatusFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'priorMedsToDateStatus_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.priorMedsToDateStatus = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'priorMedsToDateStatus_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPriorMedsToDateStatusEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.priorMedsToDateStatus = vm.item})
      }
    }
