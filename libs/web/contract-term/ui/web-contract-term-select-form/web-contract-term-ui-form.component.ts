...
export class WebFormsUiContractTermComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebContractTermFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contractTerm_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.contractTerm = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contractTerm_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadContractTermEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.contractTerm = vm.item})
      }
    }
