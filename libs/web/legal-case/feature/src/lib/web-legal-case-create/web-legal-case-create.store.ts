...
import { UserCreateLegalCaseInput, WebCoreDataAccessService, LegalCase, AccidentType,Patient,MedLevel,Firm,Attorney,User,CaseStatus,CaseType,PatientTreatmentStatus,CaseProgressStatus,AdverseInsuranceStatus } from '@case-clinical/web/core/data-access'
...
export interface LegalCaseCreateState {
  errors?: any
  loading?: boolean
  item?: LegalCase,
...
@Injectable()
export class WebLegalCaseCreateStore extends ComponentStore<LegalCaseCreateState> {
  constructor(
    ...
    input$.pipe(
      ...
      switchMap((input) =>
         this.legalCaseService.createLegalCase({...input}).pipe(
          tapResponse(
            (legalCase: LegalCase) => {
              this.patchState({ item: legalCase, loading: false })
              return this.router.navigate(['..', legalCase?.id], {relativeTo: this.route})
            },
            (errors: any) => {
