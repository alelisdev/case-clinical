...
import { WebJournalEntryTemplateCreateStore } from './web-journal-entry-template-create.store'
import { ActivatedRoute,Router } from '@angular/router'
import { FormlyFieldConfig } from '@ngx-formly/core'
import { map, pluck } from 'rxjs/operators'

@Component({
  templateUrl: './web-journal-entry-template-create.component.html',
  providers: [WebJournalEntryTemplateCreateStore],
})
export class WebJournalEntryTemplateCreateComponent {
...
WebUiFormField.number('ordinal', { label: 'Ordinal' }, {className: 'w-full sm:w-1/2 md:w-1/4  px-1'}),
WebUiFormField.input('scenario', { label: 'Scenario' }, {className: 'w-full sm:w-1/2 md:w-1/4  px-1'})
  ...
  constructor(
    private readonly store: WebJournalEntryTemplateCreateStore,
  ...
  createJournalEntryTemplate(input) {
    ...
    this.store.createJournalEntryTemplateEffect(input)
  }
}
