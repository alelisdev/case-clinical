...
import { WebCoreDataAccessService, AuthorizationStatus, CorePaging, UserUpdateAuthorizationStatusInput  } from '@case-clinical/web/core/data-access'
...
export interface AuthorizationStatusListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebAuthorizationStatusListStore extends ComponentStore<AuthorizationStatusListState> {
  constructor(
    ...
    super({
      headerTitle: 'AuthorizationStatuses',
      searchFocused: false,
      searchQuery: '',
...
readonly vm$ = this.select(
    ...
    addAuthorizationStatuses = this.updater((state, authorizationStatuses: any[]) => ({...state, data: state.data.concat(authorizationStatuses) }))
    updateAuthorizationStatuses = this.updater((state, authorizationStatuses: any[]) => {
        return {
            ...state,
            data: state.data.map((authorizationStatus) => {
            const updated = authorizationStatuses.find((el) => el.id === authorizationStatus.id);
