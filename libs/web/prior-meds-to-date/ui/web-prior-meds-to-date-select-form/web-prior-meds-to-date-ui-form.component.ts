...
export class WebFormsUiPriorMedsToDateComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebPriorMedsToDateFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'priorMedsToDate_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.priorMedsToDate = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'priorMedsToDate_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPriorMedsToDateEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.priorMedsToDate = vm.item})
      }
    }
