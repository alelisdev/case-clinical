...
export class WebFormsUiTeamRoleComponent implements OnInit, OnDestroy {
  @Input() teamRole: TeamRole
  ...
  constructor(
    public readonly store: WebTeamRoleFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'teamRole_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.teamRole = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'teamRole_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadTeamRoleEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.teamRole = vm.item})
      }
    }
