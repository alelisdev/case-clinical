...
import { WebCoreDataAccessService, TeamRole, CorePaging, UserUpdateTeamRoleInput  } from '@case-clinical/web/core/data-access'
...
export interface TeamRoleListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebTeamRoleListStore extends ComponentStore<TeamRoleListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addTeamRoles = this.updater((state, teamRoles: any[]) => ({...state, data: state.data.concat(teamRoles) }))
    updateTeamRoles = this.updater((state, teamRoles: any[]) => {
        return {
            ...state,
            data: state.data.map((teamRole) => {
            const updated = teamRoles.find((el) => el.id === teamRole.id);
