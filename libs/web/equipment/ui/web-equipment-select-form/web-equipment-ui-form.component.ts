...
export class WebFormsUiEquipmentComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebEquipmentFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'equipment_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.equipment = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'equipment_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadEquipmentEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.equipment = vm.item})
      }
    }
