...
import { WebCoreDataAccessService, Equipment, CorePaging, UserUpdateEquipmentInput  } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebEquipmentListStore extends ComponentStore<EquipmentListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addEquipments = this.updater((state, equipments: any[]) => ({...state, data: state.data.concat(equipments) }))
    updateEquipments = this.updater((state, equipments: any[]) => {
        return {
            ...state,
            data: state.data.map((equipment) => {
            const updated = equipments.find((el) => el.id === equipment.id);
