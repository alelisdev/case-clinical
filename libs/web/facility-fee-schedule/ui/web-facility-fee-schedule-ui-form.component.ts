...
import { WebFacilityFeeScheduleFormStore } from './web-facility-fee-schedule-form.store'
import { FacilityFeeSchedule,Organization,Specialty } from '@case-clinical/web/core/data-access'
import { WebUiFormField } from '@case-clinical/web/ui/form'
import { FormlyFieldConfig } from '@ngx-formly/core'
import { map, pluck, tap } from 'rxjs/operators'

@Component({
  selector: 'ui-facility-fee-schedule-form',
  providers: [WebFacilityFeeScheduleFormStore],
  template: `
    <div class="md:px-2 lg:px-0 lg:col-span-9 dark:bg-gray-800 bg-white rounded-lg shadow p-4">
      <div class="px-6 pt-6">
        <ui-form (submitForm)="submit($any(facilityFeeSchedule))" [model]="facilityFeeSchedule ?? {}" [fields]="fields" [form]="form">
          <div
...
export class WebFormsUiFacilityFeeScheduleComponent
    {
  @Input() facilityFeeSchedule: FacilityFeeSchedule = {}
...
WebUiFormField.currency('medicareFacilityRate', { label: 'Medicare Facility Rate' }, {className: 'sm:w-full md:w-1/2 lg:w-1/4  px-1'}),
WebUiFormField.currency('facilityFee', { label: 'Facility Fee' }, {className: 'sm:w-full md:w-1/2 lg:w-1/4  px-1'}),
WebUiFormField.currency('baseUnit', { label: 'Base Unit' }, {className: 'sm:w-full md:w-1/2 lg:w-1/4  px-1'}),
WebUiFormField.currency('profCf', { label: 'Prof Cf' }, {className: 'sm:w-full md:w-1/2 lg:w-1/4  px-1'})				])

]

  constructor(
    private readonly store: WebFacilityFeeScheduleFormStore,
    private readonly route: ActivatedRoute,
    private readonly ref: ChangeDetectorRef
  ) {}


  async submit({ name,organizationId,specialtyId,code,modifier,description,medicareFacilityRate,facilityFee,baseUnit,profCf }) {
    ...
    await this.store.createFacilityFeeScheduleEffect({ name,organizationId,specialtyId,code,modifier,description,medicareFacilityRate,facilityFee,baseUnit,profCf })
