...
export class WebFormsUiFacilityFeeScheduleComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebFacilityFeeScheduleFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'facilityFeeSchedule_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.facilityFeeSchedule = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'facilityFeeSchedule_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadFacilityFeeScheduleEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.facilityFeeSchedule = vm.item})
      }
    }
