...
import { WebCoreDataAccessService, CaseStatus, CorePaging, UserUpdateCaseStatusInput  } from '@case-clinical/web/core/data-access'
...
export interface CaseStatusListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebCaseStatusListStore extends ComponentStore<CaseStatusListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addCaseStatuses = this.updater((state, caseStatuses: any[]) => ({...state, data: state.data.concat(caseStatuses) }))
    updateCaseStatuses = this.updater((state, caseStatuses: any[]) => {
        return {
            ...state,
            data: state.data.map((caseStatus) => {
            const updated = caseStatuses.find((el) => el.id === caseStatus.id);
