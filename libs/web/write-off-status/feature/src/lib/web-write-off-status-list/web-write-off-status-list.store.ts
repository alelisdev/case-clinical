...
import { WebCoreDataAccessService, WriteOffStatus, CorePaging, UserUpdateWriteOffStatusInput  } from '@case-clinical/web/core/data-access'
...
export interface WriteOffStatusListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebWriteOffStatusListStore extends ComponentStore<WriteOffStatusListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addWriteOffStatuses = this.updater((state, writeOffStatuses: any[]) => ({...state, data: state.data.concat(writeOffStatuses) }))
    updateWriteOffStatuses = this.updater((state, writeOffStatuses: any[]) => {
        return {
            ...state,
            data: state.data.map((writeOffStatus) => {
            const updated = writeOffStatuses.find((el) => el.id === writeOffStatus.id);
