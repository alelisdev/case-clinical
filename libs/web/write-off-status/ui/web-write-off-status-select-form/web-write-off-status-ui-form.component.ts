...
@Component({
  selector: 'ui-write-off-status-form',
  template: `
...
export class WebFormsUiWriteOffStatusComponent implements OnInit, OnDestroy {
  @Input() writeOffStatus: WriteOffStatus
  ...
  constructor(
    public readonly store: WebWriteOffStatusFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'writeOffStatus_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.writeOffStatus = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'writeOffStatus_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadWriteOffStatusEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.writeOffStatus = vm.item})
      }
    }
