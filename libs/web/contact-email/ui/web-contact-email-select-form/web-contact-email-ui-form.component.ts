...
@Component({
  selector: 'ui-contact-email-form',
  template: `
...
export class WebFormsUiContactEmailComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebContactEmailFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contactEmail_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.contactEmail = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contactEmail_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadContactEmailEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.contactEmail = vm.item})
      }
    }
