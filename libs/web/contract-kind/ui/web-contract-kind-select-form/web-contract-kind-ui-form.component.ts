...
@Component({
  selector: 'ui-contract-kind-form',
  template: `
...
export class WebFormsUiContractKindComponent implements OnInit, OnDestroy {
  @Input() contractKind: ContractKind
  ...
  constructor(
    public readonly store: WebContractKindFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contractKind_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.contractKind = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contractKind_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadContractKindEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.contractKind = vm.item})
      }
    }
