...
import { WebCoreDataAccessService, ContractKind, CorePaging, UserUpdateContractKindInput  } from '@case-clinical/web/core/data-access'
...
export interface ContractKindListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebContractKindListStore extends ComponentStore<ContractKindListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addContractKinds = this.updater((state, contractKinds: any[]) => ({...state, data: state.data.concat(contractKinds) }))
    updateContractKinds = this.updater((state, contractKinds: any[]) => {
        return {
            ...state,
            data: state.data.map((contractKind) => {
            const updated = contractKinds.find((el) => el.id === contractKind.id);
