...
export class WebFormsUiCasePreInjuryComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebCasePreInjuryFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'casePreInjury_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.casePreInjury = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'casePreInjury_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadCasePreInjuryEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.casePreInjury = vm.item})
      }
    }
