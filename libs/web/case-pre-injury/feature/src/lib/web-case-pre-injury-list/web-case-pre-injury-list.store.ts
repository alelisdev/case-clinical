...
import { WebCoreDataAccessService, CasePreInjury, CorePaging, UserUpdateCasePreInjuryInput ,LegalCase } from '@case-clinical/web/core/data-access'
...
export interface CasePreInjuryListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebCasePreInjuryListStore extends ComponentStore<CasePreInjuryListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addCasePreInjuries = this.updater((state, casePreInjuries: any[]) => ({...state, data: state.data.concat(casePreInjuries) }))
    updateCasePreInjuries = this.updater((state, casePreInjuries: any[]) => {
        return {
            ...state,
            data: state.data.map((casePreInjury) => {
            const updated = casePreInjuries.find((el) => el.id === casePreInjury.id);
  ...
  validateImportData(excelData: any[]) {
    return this.vm$.pipe(
      switchMap((vm) => {
        ...
        return this.casePreInjuryService.validateCasePreInjuryExcelData(excelData,legalCases);
      })
    )
  }
