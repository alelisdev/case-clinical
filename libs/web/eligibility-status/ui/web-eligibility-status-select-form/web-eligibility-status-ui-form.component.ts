...
@Component({
  selector: 'ui-eligibility-status-form',
  template: `
...
export class WebFormsUiEligibilityStatusComponent implements OnInit, OnDestroy {
  @Input() eligibilityStatus: EligibilityStatus
  ...
  constructor(
    public readonly store: WebEligibilityStatusFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'eligibilityStatus_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.eligibilityStatus = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'eligibilityStatus_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadEligibilityStatusEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.eligibilityStatus = vm.item})
      }
    }
