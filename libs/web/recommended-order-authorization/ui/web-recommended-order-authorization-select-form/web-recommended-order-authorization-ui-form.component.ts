...
@Component({
  selector: 'ui-recommended-order-authorization-form',
  template: `
...
export class WebFormsUiRecommendedOrderAuthorizationComponent implements OnInit, OnDestroy {
  @Input() recommendedOrderAuthorization: RecommendedOrderAuthorization
  ...
  constructor(
    public readonly store: WebRecommendedOrderAuthorizationFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'recommendedOrderAuthorization_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({name: ''})
      this.store.vm$.subscribe((vm) => {
      this.recommendedOrderAuthorization = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'recommendedOrderAuthorization_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadRecommendedOrderAuthorizationEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.recommendedOrderAuthorization = vm.item})
      }
    }
