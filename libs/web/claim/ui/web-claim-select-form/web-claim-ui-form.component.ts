...
export class WebFormsUiClaimComponent implements OnInit, OnDestroy {
  @Input() claim: Claim
  ...
  constructor(
    public readonly store: WebClaimFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'claim_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({name: ''})
      this.store.vm$.subscribe((vm) => {
      this.claim = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'claim_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadClaimEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.claim = vm.item})
      }
    }
