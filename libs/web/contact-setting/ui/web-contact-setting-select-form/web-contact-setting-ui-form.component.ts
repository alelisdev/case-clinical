...
@Component({
  selector: 'ui-contact-setting-form',
  template: `
...
export class WebFormsUiContactSettingComponent implements OnInit, OnDestroy {
  @Input() contactSetting: ContactSetting
  ...
  constructor(
    public readonly store: WebContactSettingFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contactSetting_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.contactSetting = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contactSetting_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadContactSettingEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.contactSetting = vm.item})
      }
    }
