...
export class WebFormsUiPatientComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebPatientFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'patient_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.patient = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'patient_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPatientEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.patient = vm.item})
      }
    }
