...
export class WebFormsUiCaseAccountPaymentComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebCaseAccountPaymentFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'caseAccountPayment_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.caseAccountPayment = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'caseAccountPayment_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadCaseAccountPaymentEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.caseAccountPayment = vm.item})
      }
    }
