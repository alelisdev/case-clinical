...
export class WebFormsUiContactComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebContactFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contact_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.contact = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contact_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadContactEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.contact = vm.item})
      }
    }
