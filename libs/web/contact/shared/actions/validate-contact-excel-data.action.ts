...
import { ContactKind } from '@case-clinical/shared/util/sdk'
export class ValidateContactExcelDataAction extends ContactBusinessActionBase<boolean> {
  ...
  constructor(excelData: any[], contactKinds: ContactKind[]) {
    ...
    this.excelData.map((datum, index) => {
      names.push(datum['name']);
            ...
            this.validationContext.addRule(
              new ImportSolutionRule(`contactKindName_${index}_is_valid}`, "Contact Kind Is Not Valid", 'contactKind.name', datum['contactKind'], this.contactKinds, true)
            )
    })
