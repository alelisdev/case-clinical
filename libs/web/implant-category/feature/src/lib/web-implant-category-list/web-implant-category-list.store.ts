...
import { WebCoreDataAccessService, ImplantCategory, CorePaging, UserUpdateImplantCategoryInput  } from '@case-clinical/web/core/data-access'
...
export interface ImplantCategoryListState {
  headerTitle?: string
  errors?: any
  ...
  paging?: CorePaging
  loading?: boolean
  data?: ImplantCategory[]
  menuItems?: MenuItem[]
}

@Injectable()
export class WebImplantCategoryListStore extends ComponentStore<ImplantCategoryListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addImplantCategories = this.updater((state, implantCategories: any[]) => ({...state, data: state.data.concat(implantCategories) }))
    updateImplantCategories = this.updater((state, implantCategories: any[]) => {
        return {
            ...state,
            data: state.data.map((implantCategory) => {
            const updated = implantCategories.find((el) => el.id === implantCategory.id);
