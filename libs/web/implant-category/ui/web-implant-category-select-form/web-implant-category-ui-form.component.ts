...
@Component({
  selector: 'ui-implant-category-form',
  template: `
...
export class WebFormsUiImplantCategoryComponent implements OnInit, OnDestroy {
  @Input() implantCategory: ImplantCategory
  ...
  constructor(
    public readonly store: WebImplantCategoryFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'implantCategory_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.implantCategory = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'implantCategory_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadImplantCategoryEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.implantCategory = vm.item})
      }
    }
