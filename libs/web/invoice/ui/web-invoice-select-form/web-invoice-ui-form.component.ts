...
export class WebFormsUiInvoiceComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebInvoiceFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'invoice_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.invoice = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'invoice_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadInvoiceEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.invoice = vm.item})
      }
    }
