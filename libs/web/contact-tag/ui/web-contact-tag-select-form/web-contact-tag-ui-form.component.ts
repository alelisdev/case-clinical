...
export class WebFormsUiContactTagComponent implements OnInit, OnDestroy {
  @Input() contactTag: ContactTag
  ...
  constructor(
    public readonly store: WebContactTagFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contactTag_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.contactTag = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contactTag_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadContactTagEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.contactTag = vm.item})
      }
    }
