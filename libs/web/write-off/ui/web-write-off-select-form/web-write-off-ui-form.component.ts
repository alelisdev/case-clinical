...
export class WebFormsUiWriteOffComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebWriteOffFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'writeOff_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.writeOff = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'writeOff_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadWriteOffEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.writeOff = vm.item})
      }
    }
