...
import { WebCoreDataAccessService, DocumentType, CorePaging, UserUpdateDocumentTypeInput  } from '@case-clinical/web/core/data-access'
import { MenuItem } from '@case-clinical/web/ui/dropdown'
import { StackedListItem } from '@case-clinical/web/ui/stacked-list'
import { ComponentStore,tapResponse } from '@ngrx/component-store'
import { map, switchMap, tap, withLatestFrom, catchError } from 'rxjs/operators'
import { ColumnState, SortModelItem } from '@ag-grid-community/core'
import { DocumentTypeService } from '@case-clinical/web/document-type/shared'
...
@Injectable()
export class WebDocumentTypeListStore extends ComponentStore<DocumentTypeListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addDocumentTypes = this.updater((state, documentTypes: any[]) => ({...state, data: state.data.concat(documentTypes) }))
    updateDocumentTypes = this.updater((state, documentTypes: any[]) => {
        return {
            ...state,
            data: state.data.map((documentType) => {
            const updated = documentTypes.find((el) => el.id === documentType.id);
