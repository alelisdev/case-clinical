...
export class WebFormsUiInsuranceComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebInsuranceFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'insurance_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.insurance = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'insurance_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadInsuranceEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.insurance = vm.item})
      }
    }
