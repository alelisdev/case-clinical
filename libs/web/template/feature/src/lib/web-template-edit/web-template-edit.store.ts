...
@Injectable()
export class WebTemplateEditStore extends ComponentStore<TemplateEditState> {
  constructor(
    ...
    this.loadTemplateEffect(route.params.pipe(map((route) => route?.templateId)))
  }
  ...
  readonly updateTemplateEffect = this.effect<UserUpdateTemplateInput>((input$) =>
    input$.pipe(
      ...
      switchMap(([input, item]) =>
         this.templateService.updateTemplate(input, item?.id).pipe(
          tapResponse(
            (response: any) => {
