...
export class WebFormsUitemplateComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebTemplateFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'template_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.template = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'template_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadTemplateEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.template = vm.item})
      }
    }
