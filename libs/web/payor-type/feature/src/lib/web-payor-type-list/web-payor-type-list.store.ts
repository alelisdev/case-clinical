...
import { WebCoreDataAccessService, PayorType, CorePaging, UserUpdatePayorTypeInput  } from '@case-clinical/web/core/data-access'
...
export interface PayorTypeListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebPayorTypeListStore extends ComponentStore<PayorTypeListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addPayorTypes = this.updater((state, payorTypes: any[]) => ({...state, data: state.data.concat(payorTypes) }))
    updatePayorTypes = this.updater((state, payorTypes: any[]) => {
        return {
            ...state,
            data: state.data.map((payorType) => {
            const updated = payorTypes.find((el) => el.id === payorType.id);
