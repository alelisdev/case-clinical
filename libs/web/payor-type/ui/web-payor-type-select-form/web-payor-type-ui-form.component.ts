...
export class WebFormsUiPayorTypeComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebPayorTypeFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'payorType_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.payorType = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'payorType_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPayorTypeEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.payorType = vm.item})
      }
    }
