...
@Component({
  selector: 'ui-medical-condition-form',
  template: `
...
export class WebFormsUiMedicalConditionComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebMedicalConditionFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'medicalCondition_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.medicalCondition = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'medicalCondition_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadMedicalConditionEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.medicalCondition = vm.item})
      }
    }
