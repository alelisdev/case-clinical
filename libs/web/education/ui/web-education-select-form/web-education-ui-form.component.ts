...
export class WebFormsUiEducationComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebEducationFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'education_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.education = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'education_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadEducationEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.education = vm.item})
      }
    }
