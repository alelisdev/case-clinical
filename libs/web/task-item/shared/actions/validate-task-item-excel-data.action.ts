...
import { LegalCase, User } from '@case-clinical/shared/util/sdk'
export class ValidateTaskItemExcelDataAction extends TaskItemBusinessActionBase<boolean> {
...
assignedTos: User[]
  ...
  constructor(excelData: any[], legalCases: LegalCase[], assignedTos: User[]) {
  ...
  preValidateAction() {
    ...
    this.excelData.map((datum, index) => {
      names.push(datum['name']);
            ...
            this.validationContext.addRule(
              new ImportSolutionRule(`assignedToName_${index}_is_valid}`, "Assigned to Is Not Valid", 'assignedTo.name', datum['assignedTo'], this.assignedTos, true)
            )
    })
