...
export class WebFormsUiTaskItemComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebTaskItemFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'taskItem_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.taskItem = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'taskItem_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadTaskItemEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.taskItem = vm.item})
      }
    }
