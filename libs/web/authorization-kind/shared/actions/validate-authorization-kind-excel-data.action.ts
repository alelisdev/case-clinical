...
import { Category } from '@case-clinical/shared/util/sdk'
export class ValidateAuthorizationKindExcelDataAction extends AuthorizationKindBusinessActionBase<boolean> {
  ...
  constructor(excelData: any[], categories: Category[]) {
    ...
    this.excelData.map((datum, index) => {
      names.push(datum['name']);
            ...
            this.validationContext.addRule(
              new ImportSolutionRule(`categoryName_${index}_is_valid}`, "Category Is Not Valid", 'category.name', datum['category'], this.categories, true)
            )
    })
