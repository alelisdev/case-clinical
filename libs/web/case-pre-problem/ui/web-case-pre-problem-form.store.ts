...
import { WebCoreDataAccessService, CasePreProblem, UserCreateCasePreProblemInput, LegalCase } from '@case-clinical/web/core/data-access'
...
@Injectable()
export class WebCasePreProblemFormStore extends ComponentStore<CasePreProblemFormState> {
...
{debounce: true})
  ...
  readonly createCasePreProblemEffect = this.effect<UserCreateCasePreProblemInput>((input$) =>
    input$.pipe(
      ...
      switchMap((input) =>
        this.data.userCreateCasePreProblem({ input }).pipe(
          tapResponse(
            (res) => {
