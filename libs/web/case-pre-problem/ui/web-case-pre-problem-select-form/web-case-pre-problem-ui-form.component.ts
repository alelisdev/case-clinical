...
export class WebFormsUiCasePreProblemComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebCasePreProblemFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'casePreProblem_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.casePreProblem = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'casePreProblem_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadCasePreProblemEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.casePreProblem = vm.item})
      }
    }
