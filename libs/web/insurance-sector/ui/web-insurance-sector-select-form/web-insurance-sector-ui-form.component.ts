...
@Component({
  selector: 'ui-insurance-sector-form',
  template: `
...
export class WebFormsUiInsuranceSectorComponent implements OnInit, OnDestroy {
  @Input() insuranceSector: InsuranceSector
  ...
  constructor(
    public readonly store: WebInsuranceSectorFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'insuranceSector_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.insuranceSector = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'insuranceSector_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadInsuranceSectorEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.insuranceSector = vm.item})
      }
    }
