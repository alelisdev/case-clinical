...
export class WebFormsUiManufacturerComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebManufacturerFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'manufacturer_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.manufacturer = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'manufacturer_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadManufacturerEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.manufacturer = vm.item})
      }
    }
