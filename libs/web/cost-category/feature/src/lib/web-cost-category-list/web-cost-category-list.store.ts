...
import { WebCoreDataAccessService, CostCategory, CorePaging, UserUpdateCostCategoryInput  } from '@case-clinical/web/core/data-access'
...
export interface CostCategoryListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebCostCategoryListStore extends ComponentStore<CostCategoryListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addCostCategories = this.updater((state, costCategories: any[]) => ({...state, data: state.data.concat(costCategories) }))
    updateCostCategories = this.updater((state, costCategories: any[]) => {
        return {
            ...state,
            data: state.data.map((costCategory) => {
            const updated = costCategories.find((el) => el.id === costCategory.id);
