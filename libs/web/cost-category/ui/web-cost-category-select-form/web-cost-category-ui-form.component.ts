...
@Component({
  selector: 'ui-cost-category-form',
  template: `
...
export class WebFormsUiCostCategoryComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebCostCategoryFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'costCategory_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.costCategory = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'costCategory_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadCostCategoryEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.costCategory = vm.item})
      }
    }
