...
export class WebFormsUiContractComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebContractFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contract_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.contract = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contract_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadContractEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.contract = vm.item})
      }
    }
