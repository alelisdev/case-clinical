...
import { WebUserFeatureFormStore } from './web-user-feature-form.store'
import { UserFeature,Feature,User } from '@case-clinical/web/core/data-access'
import { WebUiFormField } from '@case-clinical/web/ui/form'
import { FormlyFieldConfig } from '@ngx-formly/core'
import { map, pluck, tap } from 'rxjs/operators'

@Component({
  selector: 'ui-user-feature-form',
  providers: [WebUserFeatureFormStore],
  template: `
    <div class="md:px-2 lg:px-0 lg:col-span-9 dark:bg-gray-800 bg-white rounded-lg shadow p-4">
      <div class="px-6 pt-6">
        <ui-form (submitForm)="submit($any(userFeature))" [model]="userFeature ?? {}" [fields]="fields" [form]="form">
          <div
...
export class WebFormsUiUserFeatureComponent
    {
  @Input() userFeature: UserFeature = {}
...
className: 'w-full sm:w-1/2 md:w-1/4 px-1'
  ...
  constructor(
    private readonly store: WebUserFeatureFormStore,
    private readonly route: ActivatedRoute,
    private readonly ref: ChangeDetectorRef
  ) {}


  async submit({ name,featureId,userId }) {
    await this.store.createUserFeatureEffect({ name,featureId,userId })
