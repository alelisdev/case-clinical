...
import { UserUpdateTemplateInput, WebCoreDataAccessService, Template, User,  } from '@case-clinical/web/core/data-access'
        ...
        this.data.userTemplate({ templateId }).pipe(
          tapResponse(
            (res) => {
              ...
              console.log(JSON.parse(res.data.item.code))
              
              this.patchState({ item: {...res.data.item, 
                                       code: ((res.data.item.code) ? JSON.parse(res.data.item.code) : {name: 'default'})}, errors: res.errors, loading: false })
            },
            (errors: any) =>
              this.patchState({
                loading: false,
