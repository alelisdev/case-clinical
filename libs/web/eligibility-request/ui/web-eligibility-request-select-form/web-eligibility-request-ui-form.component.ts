...
@Component({
  selector: 'ui-eligibility-request-form',
  template: `
...
export class WebFormsUiEligibilityRequestComponent implements OnInit, OnDestroy {
  @Input() eligibilityRequest: EligibilityRequest
  ...
  constructor(
    public readonly store: WebEligibilityRequestFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'eligibilityRequest_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.eligibilityRequest = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'eligibilityRequest_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadEligibilityRequestEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.eligibilityRequest = vm.item})
      }
    }
