...
@Component({
  selector: 'ui-clinical-provider-specialty-form',
  template: `
...
export class WebFormsUiClinicalProviderSpecialtyComponent implements OnInit, OnDestroy {
  @Input() clinicalProviderSpecialty: ClinicalProviderSpecialty
  ...
  constructor(
    public readonly store: WebClinicalProviderSpecialtyFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'clinicalProviderSpecialty_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.clinicalProviderSpecialty = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'clinicalProviderSpecialty_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadClinicalProviderSpecialtyEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.clinicalProviderSpecialty = vm.item})
      }
    }
