...
@Component({
  selector: 'ui-body-part-lead-form',
  template: `
...
export class WebFormsUiBodyPartLeadComponent implements OnInit, OnDestroy {
  @Input() bodyPartLead: BodyPartLead
  ...
  constructor(
    public readonly store: WebBodyPartLeadFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'bodyPartLead_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.bodyPartLead = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'bodyPartLead_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadBodyPartLeadEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.bodyPartLead = vm.item})
      }
    }
