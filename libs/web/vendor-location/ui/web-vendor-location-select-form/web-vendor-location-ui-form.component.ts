...
@Component({
  selector: 'ui-vendor-location-form',
  template: `
...
export class WebFormsUiVendorLocationComponent implements OnInit, OnDestroy {
  @Input() vendorLocation: VendorLocation
  ...
  constructor(
    public readonly store: WebVendorLocationFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'vendorLocation_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.vendorLocation = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'vendorLocation_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadVendorLocationEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.vendorLocation = vm.item})
      }
    }
