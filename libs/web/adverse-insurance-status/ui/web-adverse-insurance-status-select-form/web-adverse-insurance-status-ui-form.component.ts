...
@Component({
  selector: 'ui-adverse-insurance-status-form',
  template: `
...
export class WebFormsUiAdverseInsuranceStatusComponent implements OnInit, OnDestroy {
  @Input() adverseInsuranceStatus: AdverseInsuranceStatus
  ...
  constructor(
    public readonly store: WebAdverseInsuranceStatusFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'adverseInsuranceStatus_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.adverseInsuranceStatus = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'adverseInsuranceStatus_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadAdverseInsuranceStatusEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.adverseInsuranceStatus = vm.item})
      }
    }
