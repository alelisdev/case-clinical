...
export class WebFormsUiSpecialtyComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebSpecialtyFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'specialty_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.specialty = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'specialty_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadSpecialtyEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.specialty = vm.item})
      }
    }
