...
import { WebCoreDataAccessService, Specialty, CorePaging, UserUpdateSpecialtyInput  } from '@case-clinical/web/core/data-access'
import { MenuItem } from '@case-clinical/web/ui/dropdown'
import { StackedListItem } from '@case-clinical/web/ui/stacked-list'
import { ComponentStore,tapResponse } from '@ngrx/component-store'
import { map, switchMap, tap, withLatestFrom, catchError } from 'rxjs/operators'
import { ColumnState, SortModelItem } from '@ag-grid-community/core'
import { SpecialtyService } from '@case-clinical/web/specialty/shared'
...
@Injectable()
export class WebSpecialtyListStore extends ComponentStore<SpecialtyListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addSpecialties = this.updater((state, specialties: any[]) => ({...state, data: state.data.concat(specialties) }))
    updateSpecialties = this.updater((state, specialties: any[]) => {
        return {
            ...state,
            data: state.data.map((specialty) => {
            const updated = specialties.find((el) => el.id === specialty.id);
