...
export class WebFormsUiTreatmentComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebTreatmentFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'treatment_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.treatment = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'treatment_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadTreatmentEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.treatment = vm.item})
      }
    }
