...
import { WebCoreDataAccessService, BatchControl, CorePaging, UserUpdateBatchControlInput  } from '@case-clinical/web/core/data-access'
...
export interface BatchControlListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebBatchControlListStore extends ComponentStore<BatchControlListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addBatchControls = this.updater((state, batchControls: any[]) => ({...state, data: state.data.concat(batchControls) }))
    updateBatchControls = this.updater((state, batchControls: any[]) => {
        return {
            ...state,
            data: state.data.map((batchControl) => {
            const updated = batchControls.find((el) => el.id === batchControl.id);
