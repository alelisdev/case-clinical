...
export class WebFormsUiPaymentTypeComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebPaymentTypeFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'paymentType_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.paymentType = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'paymentType_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPaymentTypeEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.paymentType = vm.item})
      }
    }
