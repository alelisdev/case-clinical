...
import { WebCoreDataAccessService, PaymentType, CorePaging, UserUpdatePaymentTypeInput  } from '@case-clinical/web/core/data-access'
...
export interface PaymentTypeListState {
  headerTitle?: string
  errors?: any
...
@Injectable()
export class WebPaymentTypeListStore extends ComponentStore<PaymentTypeListState> {
  constructor(
...
readonly vm$ = this.select(
    ...
    addPaymentTypes = this.updater((state, paymentTypes: any[]) => ({...state, data: state.data.concat(paymentTypes) }))
    updatePaymentTypes = this.updater((state, paymentTypes: any[]) => {
        return {
            ...state,
            data: state.data.map((paymentType) => {
            const updated = paymentTypes.find((el) => el.id === paymentType.id);
