...
import { WebImplantDetailStore } from './web-implant-detail.store'
import {
...
@Component({
  templateUrl: `./web-implant-detail.component.html`,
  providers: [WebImplantDetailStore],
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class WebImplantDetailComponent implements AfterViewInit {
  @ViewChild('matDrawer', { static: true }) matDrawer: MatDrawer
  drawerMode: 'side' | 'over'
  drawerOpened: boolean
  menuData: FuseNavigationItem[] = []
  private _unsubscribeAll: Subject<any> = new Subject<any>()
  private _item: Implant
  readonly vm$ = this.store.vm$

  constructor(
    private readonly store: WebImplantDetailStore,
                ...
                },
{
  ...
  deleteItem(item) {
    if (confirm('Are you sure?')) {
      this.store.deleteImplantEffect(item)
    }
  }
}
