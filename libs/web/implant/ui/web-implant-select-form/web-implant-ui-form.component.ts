...
export class WebFormsUiImplantComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebImplantFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'implant_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.implant = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'implant_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadImplantEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.implant = vm.item})
      }
    }
