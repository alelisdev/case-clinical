...
export class WebFormsUiPaymentComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebPaymentFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'payment_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.payment = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'payment_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPaymentEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.payment = vm.item})
      }
    }
