...
@Component({
  selector: 'ui-contact-phone-number-form',
  template: `
...
export class WebFormsUiContactPhoneNumberComponent implements OnInit, OnDestroy {
  @Input() contactPhoneNumber: ContactPhoneNumber
  ...
  constructor(
    public readonly store: WebContactPhoneNumberFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contactPhoneNumber_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.contactPhoneNumber = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'contactPhoneNumber_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadContactPhoneNumberEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.contactPhoneNumber = vm.item})
      }
    }
