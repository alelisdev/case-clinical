...
export class WebFormsUiReviewComponent implements OnInit, OnDestroy {
  @Input() review: Review
  ...
  constructor(
    public readonly store: WebReviewFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'review_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({id: null, name: ''})
      this.store.vm$.subscribe((vm) => {
      this.review = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'review_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadReviewEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.review = vm.item})
      }
    }
