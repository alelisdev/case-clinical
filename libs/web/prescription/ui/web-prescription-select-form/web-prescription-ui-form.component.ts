...
export class WebFormsUiPrescriptionComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebPrescriptionFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'prescription_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.prescription = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'prescription_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPrescriptionEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.prescription = vm.item})
      }
    }
