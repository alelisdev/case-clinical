...
@Component({
  selector: 'ui-medical-record-form',
  template: `
...
export class WebFormsUiMedicalRecordComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebMedicalRecordFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'medicalRecord_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.medicalRecord = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'medicalRecord_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadMedicalRecordEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.medicalRecord = vm.item})
      }
    }
