...
export class WebFormsUiPriorAuthorizationEquipmentComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public readonly store: WebPriorAuthorizationEquipmentFeatureStore,
   ...
   ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'priorAuthorizationEquipment_create') {
      console.log(changes.formName.currentValue)
      this.store.setItem({ name: '' })
      this.store.vm$.subscribe((vm) => {
      this.priorAuthorizationEquipment = vm.item})
    } else if (changes.formName && changes.formName.currentValue && changes.formName.currentValue == 'priorAuthorizationEquipment_edit') {
      if(changes.formControl && changes.formControl.currentValue) {
        console.log(changes.formControl.currentValue)
        this.store.loadPriorAuthorizationEquipmentEffect(changes.formControl.currentValue.value)
        this.store.vm$.subscribe((vm) => {
        this.priorAuthorizationEquipment = vm.item})
      }
    }
