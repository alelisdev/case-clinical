...
export class ApiTemplateFeatureAdminResolver {
  ...
  adminTemplateAssignedDocuments(
    @CtxUser() admin: User,
    @Args({ name: 'input', type: () => AdminListAssignedDocumentInput, nullable: true }) input?: AdminListAssignedDocumentInput,
  ) {
    return this.service.adminTemplateAssignedDocuments(admin.id, input)
  }


  @Query(() => CorePaging, { nullable: true })
  adminCountTemplateAssignedDocuments(
    @CtxUser() admin: User,
    @Args({ name: 'input', type: () => AdminListAssignedDocumentInput, nullable: true }) input?: AdminListAssignedDocumentInput,
  ) {
    return this.service.adminCountTemplateAssignedDocuments(admin.id, input)
  }
