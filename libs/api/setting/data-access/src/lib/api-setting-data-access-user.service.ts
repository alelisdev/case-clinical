...
export class ApiSettingDataAccessUserService {
  ...
  async userCreateSetting(userId: string, input: UserCreateSettingInput) {
    ...
    if(!input.userId) input = {...input, userId};
    try {
      if(input.name !== "formly_Page_Settings"){
    ...
    try {
      if (!settingId && input.id) {
        ...
        if(input.name !== "formly_Page_Settings"){
          ...
          if (settingData.length > 0) {
            if (settingData[0].id != settingId) {
              throw new ConflictException('Record must be unique.')
            }
          }
        }

        await this.data.logEvent(sendingUser, true, 'Setting', 'Update', input)
        ...
        let setting = this.data.setting.upsert({
          where: { id: settingId },
          create: data,
          update: data,
        })

        await this.data.logEvent(sendingUser, false, 'Setting', 'Update', setting)
      ...
      throw new InternalServerErrorException('Error in updating Setting')
    }
  }
  ...
  async userUpdateSettings(userId: string, input: UserUpdateSettingsInput): Promise<UpdateResult> {
    ...
    for (const key in input.settings) {
      ...
      const settingData = await this.checkSettingExist(inputData.name)
