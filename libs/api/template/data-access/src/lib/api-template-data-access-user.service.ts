...
export class ApiTemplateDataAccessUserService {
  ...
  async userTemplate(userId: string, templateId) {
    const theTemplate = await this.data.template.findUnique({ where: { id: templateId }, include: { assignedDocuments: true } })
    ...
    try {
      ...
      const containerName = 'base-project-files'
  ...
  async getPdfDocument(userId, attachment) {
    ...
    this.http.post('https://documentserver.schemadriven.com:443/DownloadPdf', mergeDocument).subscribe((res: any) => {
      let pdf = res.document
      let doc: UserCreateDocumentInput = {
        name: user.name + ' ' + 'Waiver',
        attachment: pdf,
