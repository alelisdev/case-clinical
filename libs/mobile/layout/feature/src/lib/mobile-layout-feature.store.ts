...
import { Role } from '@case-clinical/mobile/core/data-access'
...
export class MobileLayoutFeatureStore extends ComponentStore<MobileLayoutFeatureState> {
  constructor(private readonly authStore: MobileAuthStore) {
    super({
      theme: 'dark',
      logo: '/assets/images/case-clinical-logo.svg',
      title: 'Case Clinical Underwriting',
      subTitle: 'Where Legal Meets Medical',
  ...
  readonly links$ = this.select(this.authStore.user$, this.state$, (user, state) => ({
    main: state.links.filter((l) => (l.role ? l.role.name === user.role : l)),
  }))
