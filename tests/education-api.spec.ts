...
test('UserEducations Query Test', async ({ page }) => {
  const query = `
...
test('UserCountEducations Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectEducations Query Test', async ({ page }) => {
  const query = `
...
test('UserEducation Query Test', async ({ page }) => {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const educationsResponse = await sendGraphQLQuery(url.href,educationsQuery);

  // Verify the response data
  const { data: { items: educations }, errors: educationsErrors } = educationsResponse;
  expect(educationsErrors).toBeUndefined();
...
clinicalProvider {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { educationId });
...
test('UserCreateEducation Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { educationId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { educationId: id });
