...
test('UserContacts Query Test', async ({ page }) => {
  const query = `
...
test('UserCountContacts Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectContacts Query Test', async ({ page }) => {
  const query = `
...
test('UserContact Query Test', async ({ page }) => {
  const contactsQuery = `
    query UserContacts {
      items: userContacts {
        id
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const contactsResponse = await sendGraphQLQuery(url.href,contactsQuery);

  // Verify the response data
  const { data: { items: contacts }, errors: contactsErrors } = contactsResponse;
  expect(contactsErrors).toBeUndefined();
...
contactSettings {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { contactId });
...
test('UserCreateContact Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { contactId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { contactId: id });
