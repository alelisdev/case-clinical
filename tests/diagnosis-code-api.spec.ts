...
test('UserDiagnosisCodes Query Test', async ({ page }) => {
  const query = `
...
test('UserCountDiagnosisCodes Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectDiagnosisCodes Query Test', async ({ page }) => {
  const query = `
...
test('UserDiagnosisCode Query Test', async ({ page }) => {
  const diagnosisCodesQuery = `
    query UserDiagnosisCodes {
      items: userDiagnosisCodes {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const diagnosisCodesResponse = await sendGraphQLQuery(url.href,diagnosisCodesQuery);

  // Verify the response data
  const { data: { items: diagnosisCodes }, errors: diagnosisCodesErrors } = diagnosisCodesResponse;
  expect(diagnosisCodesErrors).toBeUndefined();
  expect(diagnosisCodes).toBeDefined();
  expect(diagnosisCodes.length).toBeGreaterThan(0);

  // Use the first accident type ID for the subsequent user accident type query
  const diagnosisCodeId = diagnosisCodes[0].id;
...
priorAuthorizationDiagnosisCodes {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { diagnosisCodeId });
...
test('UserCreateDiagnosisCode Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { diagnosisCodeId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { diagnosisCodeId: id });
