...
test('UserPermissions Query Test', async ({ page }) => {
  const query = `
...
test('UserCountPermissions Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectPermissions Query Test', async ({ page }) => {
  const query = `
...
test('UserPermission Query Test', async ({ page }) => {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const permissionsResponse = await sendGraphQLQuery(url.href,permissionsQuery);

  // Verify the response data
  const { data: { items: permissions }, errors: permissionsErrors } = permissionsResponse;
  expect(permissionsErrors).toBeUndefined();
...
featurePermissions {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { permissionId });
...
test('UserCreatePermission Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { permissionId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { permissionId: id });
