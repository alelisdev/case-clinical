...
test('UserDocumentTypes Query Test', async ({ page }) => {
  const query = `
...
test('UserCountDocumentTypes Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectDocumentTypes Query Test', async ({ page }) => {
  const query = `
...
test('UserDocumentType Query Test', async ({ page }) => {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const documentTypesResponse = await sendGraphQLQuery(url.href,documentTypesQuery);

  // Verify the response data
  const { data: { items: documentTypes }, errors: documentTypesErrors } = documentTypesResponse;
  expect(documentTypesErrors).toBeUndefined();
...
assignedDocuments {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { documentTypeId });
...
test('UserCreateDocumentType Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { documentTypeId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { documentTypeId: id });
