...
test('UserProcedures Query Test', async ({ page }) => {
  const query = `
...
test('UserCountProcedures Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectProcedures Query Test', async ({ page }) => {
  const query = `
...
test('UserProcedure Query Test', async ({ page }) => {
  const proceduresQuery = `
    query UserProcedures {
      items: userProcedures {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const proceduresResponse = await sendGraphQLQuery(url.href,proceduresQuery);

  // Verify the response data
  const { data: { items: procedures }, errors: proceduresErrors } = proceduresResponse;
  expect(proceduresErrors).toBeUndefined();
...
priorAuthorizationProcedureCodes {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { procedureId });
...
test('UserCreateProcedure Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { procedureId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { procedureId: id });
