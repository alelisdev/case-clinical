...
test('UserPortfolios Query Test', async ({ page }) => {
  const query = `
...
test('UserCountPortfolios Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectPortfolios Query Test', async ({ page }) => {
  const query = `
...
test('UserPortfolio Query Test', async ({ page }) => {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const portfoliosResponse = await sendGraphQLQuery(url.href,portfoliosQuery);

  // Verify the response data
  const { data: { items: portfolios }, errors: portfoliosErrors } = portfoliosResponse;
  expect(portfoliosErrors).toBeUndefined();
...
caseAccounts {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { portfolioId });
...
test('UserCreatePortfolio Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { portfolioId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { portfolioId: id });
