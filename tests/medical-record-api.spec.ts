...
test('UserMedicalRecords Query Test', async ({ page }) => {
  const query = `
...
test('UserCountMedicalRecords Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectMedicalRecords Query Test', async ({ page }) => {
  const query = `
...
test('UserMedicalRecord Query Test', async ({ page }) => {
  const medicalRecordsQuery = `
    query UserMedicalRecords {
      items: userMedicalRecords {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const medicalRecordsResponse = await sendGraphQLQuery(url.href,medicalRecordsQuery);

  // Verify the response data
  const { data: { items: medicalRecords }, errors: medicalRecordsErrors } = medicalRecordsResponse;
  expect(medicalRecordsErrors).toBeUndefined();
...
clinicalProvider {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { medicalRecordId });
...
test('UserCreateMedicalRecord Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { medicalRecordId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { medicalRecordId: id });
