...
test('UserContactSettings Query Test', async ({ page }) => {
  const query = `
...
test('UserCountContactSettings Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectContactSettings Query Test', async ({ page }) => {
  const query = `
...
test('UserContactSetting Query Test', async ({ page }) => {
  const contactSettingsQuery = `
    query UserContactSettings {
      items: userContactSettings {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const contactSettingsResponse = await sendGraphQLQuery(url.href,contactSettingsQuery);

  // Verify the response data
  const { data: { items: contactSettings }, errors: contactSettingsErrors } = contactSettingsResponse;
  expect(contactSettingsErrors).toBeUndefined();
  expect(contactSettings).toBeDefined();
  expect(contactSettings.length).toBeGreaterThan(0);

  // Use the first accident type ID for the subsequent user accident type query
  const contactSettingId = contactSettings[0].id;
...
integration {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { contactSettingId });
...
test('UserCreateContactSetting Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { contactSettingId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { contactSettingId: id });
