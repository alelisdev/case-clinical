...
test('UserLeadSources Query Test', async ({ page }) => {
  const query = `
...
test('UserCountLeadSources Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectLeadSources Query Test', async ({ page }) => {
  const query = `
...
test('UserLeadSource Query Test', async ({ page }) => {
  const leadSourcesQuery = `
    query UserLeadSources {
      items: userLeadSources {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const leadSourcesResponse = await sendGraphQLQuery(url.href,leadSourcesQuery);

  // Verify the response data
  const { data: { items: leadSources }, errors: leadSourcesErrors } = leadSourcesResponse;
  expect(leadSourcesErrors).toBeUndefined();
  expect(leadSources).toBeDefined();
  expect(leadSources.length).toBeGreaterThan(0);

  // Use the first accident type ID for the subsequent user accident type query
  const leadSourceId = leadSources[0].id;
...
leads {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { leadSourceId });
...
test('UserCreateLeadSource Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { leadSourceId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { leadSourceId: id });
