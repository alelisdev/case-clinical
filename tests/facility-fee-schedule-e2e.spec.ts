...
for (const {name,width, height } of DIMENSIONS) {
    ...
    test('take screenshot on ' + name + ' viewport of Facility Fee Schedule list', async({ page }) => {
        test.setTimeout(60000)
        await page.setViewportSize({
            height,
            width,
        })
        await page.goto(`${SETTINGS.ENVIRONMENT_URL}/sign-in?redirectURL=%2Fqueues%2Ffacility-fee-schedules`);
        ...
        await page.screenshot({
          path: `./test-results/facility-fee-schedules-list-${name}.png`,
        })
    })
}
