...
test('UserInvoices Query Test', async ({ page }) => {
  const query = `
...
test('UserCountInvoices Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectInvoices Query Test', async ({ page }) => {
  const query = `
...
test('UserInvoice Query Test', async ({ page }) => {
  const invoicesQuery = `
    query UserInvoices {
      items: userInvoices {
        id
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const invoicesResponse = await sendGraphQLQuery(url.href,invoicesQuery);

  // Verify the response data
  const { data: { items: invoices }, errors: invoicesErrors } = invoicesResponse;
  expect(invoicesErrors).toBeUndefined();
...
details {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { invoiceId });
...
test('UserCreateInvoice Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { invoiceId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { invoiceId: id });
