...
test('UserContractTerms Query Test', async ({ page }) => {
  const query = `
...
test('UserCountContractTerms Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectContractTerms Query Test', async ({ page }) => {
  const query = `
...
test('UserContractTerm Query Test', async ({ page }) => {
  const contractTermsQuery = `
    query UserContractTerms {
      items: userContractTerms {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const contractTermsResponse = await sendGraphQLQuery(url.href,contractTermsQuery);

  // Verify the response data
  const { data: { items: contractTerms }, errors: contractTermsErrors } = contractTermsResponse;
  expect(contractTermsErrors).toBeUndefined();
  expect(contractTerms).toBeDefined();
  expect(contractTerms.length).toBeGreaterThan(0);

  // Use the first accident type ID for the subsequent user accident type query
  const contractTermId = contractTerms[0].id;
...
contract {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { contractTermId });
...
test('UserCreateContractTerm Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { contractTermId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { contractTermId: id });
