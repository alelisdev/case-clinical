...
test('UserWriteOffs Query Test', async ({ page }) => {
  const query = `
...
test('UserCountWriteOffs Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectWriteOffs Query Test', async ({ page }) => {
  const query = `
...
test('UserWriteOff Query Test', async ({ page }) => {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const writeOffsResponse = await sendGraphQLQuery(url.href,writeOffsQuery);

  // Verify the response data
  const { data: { items: writeOffs }, errors: writeOffsErrors } = writeOffsResponse;
  expect(writeOffsErrors).toBeUndefined();
  expect(writeOffs).toBeDefined();
  expect(writeOffs.length).toBeGreaterThan(0);

  // Use the first accident type ID for the subsequent user accident type query
  const writeOffId = writeOffs[0].id;
...
writeOffStatus {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { writeOffId });
...
test('UserCreateWriteOff Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { writeOffId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { writeOffId: id });
