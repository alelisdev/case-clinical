...
test('UserTreatments Query Test', async ({ page }) => {
  const query = gql`
...
test('UserCountTreatments Query Test', async ({ page }) => {
  const query = gql`
...
test('UserSelectTreatments Query Test', async ({ page }) => {
  const query = gql`
...
test('UserTreatment Query Test', async ({ page }) => {
  const treatmentsQuery = gql`
    query UserTreatments {
      items: userTreatments {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const treatmentsResponse = await sendGraphQLQuery(url.href,treatmentsQuery);

  // Verify the response data
  const { data: { items: treatments }, errors: treatmentsErrors } = treatmentsResponse;
  expect(treatmentsErrors).toBeUndefined();
...
leads {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { treatmentId });
...
test('UserCreateTreatment Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { treatmentId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { treatmentId: id });
