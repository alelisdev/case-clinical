...
for (const {name,width, height } of DIMENSIONS) {
    ...
    test('take screenshot on ' + name + ' viewport of Firm list', async({ page }) => {
        test.setTimeout(60000)
        await page.setViewportSize({
            height,
            width,
        })
        await page.goto(`${SETTINGS.ENVIRONMENT_URL}/sign-in?redirectURL=%2Fqueues%2Ffirms`);
        ...
        await page.screenshot({
          path: `./test-results/firms-list-${name}.png`,
        })
    })
}
