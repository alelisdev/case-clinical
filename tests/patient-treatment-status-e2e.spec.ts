...
for (const {name,width, height } of DIMENSIONS) {
    ...
    test('take screenshot on ' + name + ' viewport of Patient Treatment Status list', async({ page }) => {
        test.setTimeout(60000)
        await page.setViewportSize({
            height,
            width,
        })
        await page.goto(`${SETTINGS.ENVIRONMENT_URL}/sign-in?redirectURL=%2Fqueues%2Fpatient-treatment-statuses`);
        ...
        await page.screenshot({
          path: `./test-results/patient-treatment-statuses-list-${name}.png`,
        })
    })
}
