...
test('UserPaymentTypes Query Test', async ({ page }) => {
  const query = `
...
test('UserCountPaymentTypes Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectPaymentTypes Query Test', async ({ page }) => {
  const query = `
...
test('UserPaymentType Query Test', async ({ page }) => {
  const paymentTypesQuery = `
    query UserPaymentTypes {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const paymentTypesResponse = await sendGraphQLQuery(url.href,paymentTypesQuery);

  // Verify the response data
  const { data: { items: paymentTypes }, errors: paymentTypesErrors } = paymentTypesResponse;
  expect(paymentTypesErrors).toBeUndefined();
...
payments {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { paymentTypeId });
...
test('UserCreatePaymentType Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { paymentTypeId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { paymentTypeId: id });
