...
test('UserUserRoles Query Test', async ({ page }) => {
  const query = `
...
test('UserCountUserRoles Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectUserRoles Query Test', async ({ page }) => {
  const query = `
...
test('UserUserRole Query Test', async ({ page }) => {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const userRolesResponse = await sendGraphQLQuery(url.href,userRolesQuery);

  // Verify the response data
  const { data: { items: userRoles }, errors: userRolesErrors } = userRolesResponse;
  expect(userRolesErrors).toBeUndefined();
...
user {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { userRoleId });
...
test('UserCreateUserRole Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { userRoleId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { userRoleId: id });
