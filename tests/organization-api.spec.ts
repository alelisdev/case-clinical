...
test('UserOrganizations Query Test', async ({ page }) => {
  const query = `
...
test('UserCountOrganizations Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectOrganizations Query Test', async ({ page }) => {
  const query = `
...
test('UserOrganization Query Test', async ({ page }) => {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const organizationsResponse = await sendGraphQLQuery(url.href,organizationsQuery);

  // Verify the response data
  const { data: { items: organizations }, errors: organizationsErrors } = organizationsResponse;
  expect(organizationsErrors).toBeUndefined();
...
liensHeld {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { organizationId });
...
test('UserCreateOrganization Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { organizationId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { organizationId: id });
