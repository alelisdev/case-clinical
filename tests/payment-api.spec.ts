...
test('UserPayments Query Test', async ({ page }) => {
  const query = `
...
test('UserCountPayments Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectPayments Query Test', async ({ page }) => {
  const query = `
...
test('UserPayment Query Test', async ({ page }) => {
  const paymentsQuery = `
    query UserPayments {
      items: userPayments {
        id
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const paymentsResponse = await sendGraphQLQuery(url.href,paymentsQuery);

  // Verify the response data
  const { data: { items: payments }, errors: paymentsErrors } = paymentsResponse;
  expect(paymentsErrors).toBeUndefined();
...
caseAccountPayments {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { paymentId });
...
test('UserCreatePayment Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { paymentId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { paymentId: id });
