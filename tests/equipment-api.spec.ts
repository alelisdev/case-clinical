...
test('UserEquipments Query Test', async ({ page }) => {
  const query = `
...
test('UserCountEquipments Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectEquipments Query Test', async ({ page }) => {
  const query = `
...
test('UserEquipment Query Test', async ({ page }) => {
  const equipmentsQuery = `
    query UserEquipments {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const equipmentsResponse = await sendGraphQLQuery(url.href,equipmentsQuery);

  // Verify the response data
  const { data: { items: equipments }, errors: equipmentsErrors } = equipmentsResponse;
  expect(equipmentsErrors).toBeUndefined();
...
priorAuthorizationEquipments {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { equipmentId });
...
test('UserCreateEquipment Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { equipmentId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { equipmentId: id });
