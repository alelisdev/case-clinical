...
test('UserLanguages Query Test', async ({ page }) => {
  const query = `
...
test('UserCountLanguages Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectLanguages Query Test', async ({ page }) => {
  const query = `
...
test('UserLanguage Query Test', async ({ page }) => {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const languagesResponse = await sendGraphQLQuery(url.href,languagesQuery);

  // Verify the response data
  const { data: { items: languages }, errors: languagesErrors } = languagesResponse;
  expect(languagesErrors).toBeUndefined();
...
patients {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { languageId });
...
test('UserCreateLanguage Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { languageId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { languageId: id });
