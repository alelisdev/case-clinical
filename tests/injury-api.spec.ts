...
test('UserInjuries Query Test', async ({ page }) => {
  const query = `
...
test('UserCountInjuries Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectInjuries Query Test', async ({ page }) => {
  const query = `
...
test('UserInjury Query Test', async ({ page }) => {
  const injuriesQuery = `
    query UserInjuries {
      items: userInjuries {
        id
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const injuriesResponse = await sendGraphQLQuery(url.href,injuriesQuery);

  // Verify the response data
  const { data: { items: injuries }, errors: injuriesErrors } = injuriesResponse;
  expect(injuriesErrors).toBeUndefined();
  expect(injuries).toBeDefined();
  expect(injuries.length).toBeGreaterThan(0);

  // Use the first accident type ID for the subsequent user accident type query
  const injuryId = injuries[0].id;
...
leads {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { injuryId });
...
test('UserCreateInjury Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { injuryId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { injuryId: id });
