...
test('UserCountries Query Test', async ({ page }) => {
  const query = `
...
test('UserCountCountries Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectCountries Query Test', async ({ page }) => {
  const query = `
...
test('UserCountry Query Test', async ({ page }) => {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const countriesResponse = await sendGraphQLQuery(url.href,countriesQuery);

  // Verify the response data
  const { data: { items: countries }, errors: countriesErrors } = countriesResponse;
  expect(countriesErrors).toBeUndefined();
...
contactPhoneNumbers {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { countryId });
...
test('UserCreateCountry Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { countryId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { countryId: id });
