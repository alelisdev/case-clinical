...
for (const {name,width, height } of DIMENSIONS) {
    ...
    test('take screenshot on ' + name + ' viewport of Prior Authorization Procedure Code list', async({ page }) => {
        test.setTimeout(60000)
        await page.setViewportSize({
            height,
            width,
        })
        await page.goto(`${SETTINGS.ENVIRONMENT_URL}/sign-in?redirectURL=%2Fqueues%2Fprior-authorization-procedure-codes`);
        ...
        await page.screenshot({
          path: `./test-results/prior-authorization-procedure-codes-list-${name}.png`,
        })
    })
}
