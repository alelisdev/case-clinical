...
test('UserReviews Query Test', async ({ page }) => {
  const query = `
...
test('UserCountReviews Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectReviews Query Test', async ({ page }) => {
  const query = `
...
test('UserReview Query Test', async ({ page }) => {
  const reviewsQuery = `
    query UserReviews {
      items: userReviews {
        id
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const reviewsResponse = await sendGraphQLQuery(url.href,reviewsQuery);

  // Verify the response data
  const { data: { items: reviews }, errors: reviewsErrors } = reviewsResponse;
  expect(reviewsErrors).toBeUndefined();
...
clinicalProvider {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { reviewId });
...
test('UserCreateReview Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { reviewId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { reviewId: id });
