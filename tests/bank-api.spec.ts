...
test('UserBanks Query Test', async ({ page }) => {
  const query = `
...
test('UserCountBanks Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectBanks Query Test', async ({ page }) => {
  const query = `
...
test('UserBank Query Test', async ({ page }) => {
  const banksQuery = `
    query UserBanks {
      items: userBanks {
        id
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const banksResponse = await sendGraphQLQuery(url.href,banksQuery);

  // Verify the response data
  const { data: { items: banks }, errors: banksErrors } = banksResponse;
  expect(banksErrors).toBeUndefined();
...
payments {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { bankId });
...
test('UserCreateBank Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { bankId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { bankId: id });
