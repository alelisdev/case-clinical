...
test('UserCategories Query Test', async ({ page }) => {
  const query = `
...
test('UserCountCategories Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectCategories Query Test', async ({ page }) => {
  const query = `
...
test('UserCategory Query Test', async ({ page }) => {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const categoriesResponse = await sendGraphQLQuery(url.href,categoriesQuery);

  // Verify the response data
  const { data: { items: categories }, errors: categoriesErrors } = categoriesResponse;
  expect(categoriesErrors).toBeUndefined();
...
authorizationKinds {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { categoryId });
...
test('UserCreateCategory Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { categoryId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { categoryId: id });
