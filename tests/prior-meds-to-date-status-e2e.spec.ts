...
for (const {name,width, height } of DIMENSIONS) {
    ...
    test('take screenshot on ' + name + ' viewport of Prior Meds to Date Status list', async({ page }) => {
        test.setTimeout(60000)
        await page.setViewportSize({
            height,
            width,
        })
        await page.goto(`${SETTINGS.ENVIRONMENT_URL}/sign-in?redirectURL=%2Fqueues%2Fprior-meds-to-date-statuses`);
        ...
        await page.screenshot({
          path: `./test-results/prior-meds-to-date-statuses-list-${name}.png`,
        })
    })
}
