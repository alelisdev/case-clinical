...
test('UserAwards Query Test', async ({ page }) => {
  const query = `
...
test('UserCountAwards Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectAwards Query Test', async ({ page }) => {
  const query = `
...
test('UserAward Query Test', async ({ page }) => {
  const awardsQuery = `
    query UserAwards {
      items: userAwards {
        id
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const awardsResponse = await sendGraphQLQuery(url.href,awardsQuery);

  // Verify the response data
  const { data: { items: awards }, errors: awardsErrors } = awardsResponse;
  expect(awardsErrors).toBeUndefined();
...
clinicalProvider {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { awardId });
...
test('UserCreateAward Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { awardId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { awardId: id });
