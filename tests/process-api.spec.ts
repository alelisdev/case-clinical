...
test('UserProcesses Query Test', async ({ page }) => {
  const query = `
...
test('UserCountProcesses Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectProcesses Query Test', async ({ page }) => {
  const query = `
...
test('UserProcess Query Test', async ({ page }) => {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const processesResponse = await sendGraphQLQuery(url.href,processesQuery);

  // Verify the response data
  const { data: { items: processes }, errors: processesErrors } = processesResponse;
  expect(processesErrors).toBeUndefined();
...
contracts {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { processId });
...
test('UserCreateProcess Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { processId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { processId: id });
