...
test('UserBalanceRequests Query Test', async ({ page }) => {
  const query = `
...
test('UserCountBalanceRequests Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectBalanceRequests Query Test', async ({ page }) => {
  const query = `
...
test('UserBalanceRequest Query Test', async ({ page }) => {
  const balanceRequestsQuery = `
    query UserBalanceRequests {
      items: userBalanceRequests {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const balanceRequestsResponse = await sendGraphQLQuery(url.href,balanceRequestsQuery);

  // Verify the response data
  const { data: { items: balanceRequests }, errors: balanceRequestsErrors } = balanceRequestsResponse;
  expect(balanceRequestsErrors).toBeUndefined();
  expect(balanceRequests).toBeDefined();
  expect(balanceRequests.length).toBeGreaterThan(0);

  // Use the first accident type ID for the subsequent user accident type query
  const balanceRequestId = balanceRequests[0].id;
...
legalCase {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { balanceRequestId });
...
test('UserCreateBalanceRequest Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { balanceRequestId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { balanceRequestId: id });
