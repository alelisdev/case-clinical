...
test('UserSpecialties Query Test', async ({ page }) => {
  const query = gql`
...
test('UserCountSpecialties Query Test', async ({ page }) => {
  const query = gql`
...
test('UserSelectSpecialties Query Test', async ({ page }) => {
  const query = gql`
...
test('UserSpecialty Query Test', async ({ page }) => {
  const specialtiesQuery = gql`
    query UserSpecialties {
      items: userSpecialties {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const specialtiesResponse = await sendGraphQLQuery(url.href,specialtiesQuery);

  // Verify the response data
  const { data: { items: specialties }, errors: specialtiesErrors } = specialtiesResponse;
  expect(specialtiesErrors).toBeUndefined();
...
contractedRates {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { specialtyId });
...
test('UserCreateSpecialty Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { specialtyId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { specialtyId: id });
