...
test('UserBodyParts Query Test', async ({ page }) => {
  const query = `
...
test('UserCountBodyParts Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectBodyParts Query Test', async ({ page }) => {
  const query = `
...
test('UserBodyPart Query Test', async ({ page }) => {
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const bodyPartsResponse = await sendGraphQLQuery(url.href,bodyPartsQuery);

  // Verify the response data
  const { data: { items: bodyParts }, errors: bodyPartsErrors } = bodyPartsResponse;
  expect(bodyPartsErrors).toBeUndefined();
  expect(bodyParts).toBeDefined();
  expect(bodyParts.length).toBeGreaterThan(0);

  // Use the first accident type ID for the subsequent user accident type query
  const bodyPartId = bodyParts[0].id;
...
leads {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { bodyPartId });
...
test('UserCreateBodyPart Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { bodyPartId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { bodyPartId: id });
