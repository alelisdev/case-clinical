...
for (const {name,width, height } of DIMENSIONS) {
    ...
    test('take screenshot on ' + name + ' viewport of Setting list', async({ page }) => {
        test.setTimeout(60000)
        await page.setViewportSize({
            height,
            width,
        })
        await page.goto(`${SETTINGS.ENVIRONMENT_URL}/sign-in?redirectURL=%2Fqueues%2Fsettings`);
        ...
        await page.screenshot({
          path: `./test-results/settings-list-${name}.png`,
        })
    })
}
