...
test('UserPriorAuthorizationProcedureCodes Query Test', async ({ page }) => {
  const query = `
...
test('UserCountPriorAuthorizationProcedureCodes Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectPriorAuthorizationProcedureCodes Query Test', async ({ page }) => {
  const query = `
...
test('UserPriorAuthorizationProcedureCode Query Test', async ({ page }) => {
  const priorAuthorizationProcedureCodesQuery = `
    query UserPriorAuthorizationProcedureCodes {
      items: userPriorAuthorizationProcedureCodes {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const priorAuthorizationProcedureCodesResponse = await sendGraphQLQuery(url.href,priorAuthorizationProcedureCodesQuery);

  // Verify the response data
  const { data: { items: priorAuthorizationProcedureCodes }, errors: priorAuthorizationProcedureCodesErrors } = priorAuthorizationProcedureCodesResponse;
  expect(priorAuthorizationProcedureCodesErrors).toBeUndefined();
  expect(priorAuthorizationProcedureCodes).toBeDefined();
  expect(priorAuthorizationProcedureCodes.length).toBeGreaterThan(0);

  // Use the first accident type ID for the subsequent user accident type query
  const priorAuthorizationProcedureCodeId = priorAuthorizationProcedureCodes[0].id;
...
priorAuthorizationRequest {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { priorAuthorizationProcedureCodeId });
...
test('UserCreatePriorAuthorizationProcedureCode Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { priorAuthorizationProcedureCodeId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { priorAuthorizationProcedureCodeId: id });
