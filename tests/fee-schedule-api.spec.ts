...
test('UserFeeSchedules Query Test', async ({ page }) => {
  const query = `
...
test('UserCountFeeSchedules Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectFeeSchedules Query Test', async ({ page }) => {
  const query = `
...
test('UserFeeSchedule Query Test', async ({ page }) => {
  const feeSchedulesQuery = `
    query UserFeeSchedules {
      items: userFeeSchedules {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const feeSchedulesResponse = await sendGraphQLQuery(url.href,feeSchedulesQuery);

  // Verify the response data
  const { data: { items: feeSchedules }, errors: feeSchedulesErrors } = feeSchedulesResponse;
  expect(feeSchedulesErrors).toBeUndefined();
  expect(feeSchedules).toBeDefined();
  expect(feeSchedules.length).toBeGreaterThan(0);

  // Use the first accident type ID for the subsequent user accident type query
  const feeScheduleId = feeSchedules[0].id;
...
specialty {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { feeScheduleId });
...
test('UserCreateFeeSchedule Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { feeScheduleId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { feeScheduleId: id });
