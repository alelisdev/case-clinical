...
test('UserContactEmails Query Test', async ({ page }) => {
  const query = `
...
test('UserCountContactEmails Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectContactEmails Query Test', async ({ page }) => {
  const query = `
...
test('UserContactEmail Query Test', async ({ page }) => {
  const contactEmailsQuery = `
    query UserContactEmails {
      items: userContactEmails {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const contactEmailsResponse = await sendGraphQLQuery(url.href,contactEmailsQuery);

  // Verify the response data
  const { data: { items: contactEmails }, errors: contactEmailsErrors } = contactEmailsResponse;
  expect(contactEmailsErrors).toBeUndefined();
  expect(contactEmails).toBeDefined();
  expect(contactEmails.length).toBeGreaterThan(0);

  // Use the first accident type ID for the subsequent user accident type query
  const contactEmailId = contactEmails[0].id;
...
contact {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { contactEmailId });
...
test('UserCreateContactEmail Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { contactEmailId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { contactEmailId: id });
