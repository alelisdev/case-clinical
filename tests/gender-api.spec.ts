...
test('UserGenders Query Test', async ({ page }) => {
  const query = `
...
test('UserCountGenders Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectGenders Query Test', async ({ page }) => {
  const query = `
...
test('UserGender Query Test', async ({ page }) => {
  const gendersQuery = `
    query UserGenders {
      items: userGenders {
        id
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const gendersResponse = await sendGraphQLQuery(url.href,gendersQuery);

  // Verify the response data
  const { data: { items: genders }, errors: gendersErrors } = gendersResponse;
  expect(gendersErrors).toBeUndefined();
...
patients {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { genderId });
...
test('UserCreateGender Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { genderId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { genderId: id });
