...
test('UserCostCategories Query Test', async ({ page }) => {
  const query = `
...
test('UserCountCostCategories Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectCostCategories Query Test', async ({ page }) => {
  const query = `
...
test('UserCostCategory Query Test', async ({ page }) => {
  const costCategoriesQuery = `
    query UserCostCategories {
      items: userCostCategories {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const costCategoriesResponse = await sendGraphQLQuery(url.href,costCategoriesQuery);

  // Verify the response data
  const { data: { items: costCategories }, errors: costCategoriesErrors } = costCategoriesResponse;
  expect(costCategoriesErrors).toBeUndefined();
  expect(costCategories).toBeDefined();
...
priorAuthorizationProcedureCodes {
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { costCategoryId });
...
test('UserCreateCostCategory Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { costCategoryId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { costCategoryId: id });
