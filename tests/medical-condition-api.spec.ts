...
test('UserMedicalConditions Query Test', async ({ page }) => {
  const query = `
...
test('UserCountMedicalConditions Query Test', async ({ page }) => {
  const query = `
...
test('UserSelectMedicalConditions Query Test', async ({ page }) => {
  const query = `
...
test('UserMedicalCondition Query Test', async ({ page }) => {
  const medicalConditionsQuery = `
    query UserMedicalConditions {
      items: userMedicalConditions {
        id
      }
    }
  `;
  ...
  const graphqlUrl = process.env.GRAPHQL_URL || '';
  const url = new URL(graphqlUrl);
  const medicalConditionsResponse = await sendGraphQLQuery(url.href,medicalConditionsQuery);

  // Verify the response data
  const { data: { items: medicalConditions }, errors: medicalConditionsErrors } = medicalConditionsResponse;
  expect(medicalConditionsErrors).toBeUndefined();
  expect(medicalConditions).toBeDefined();
  expect(medicalConditions.length).toBeGreaterThan(0);

  // Use the first accident type ID for the subsequent user accident type query
  const medicalConditionId = medicalConditions[0].id;
...
name
  ...
  const response = await sendGraphQLQuery(graphqlUrl, query, { medicalConditionId });
...
test('UserCreateMedicalCondition Mutation Test', async ({ page, browser }) => {
  ...
  const updateResponse = await sendGraphQLQuery(graphqlUrl, updateMutation, { medicalConditionId: id, input: updateInput });
  ...
  const deleteResponse = await sendGraphQLQuery(graphqlUrl, deleteMutation, { medicalConditionId: id });
